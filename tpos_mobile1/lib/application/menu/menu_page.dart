        ...
        return AlertDialog(
          ...
          actions: [
            ActionButton(
              ...
              onPressed: () {
                _menuBloc.add(MenuGroupRenamed(group, _controler.text.trim()));
    ...
    return BlocUiProvider<MenuBloc>(
      ...
      listen: (state) {
        if (state is MenuNotifySuccess) {
          context.showNotify(
              type: AlertDialogType.info, message: state.message);
        }
      },
      ...
      ...config.menuGroups.map((group) {
        return NormalMenuItemWrap(
          ...
          title: Text(
            ...
            style: TextStyle(
              color: group.colorString?.toColor()?.textColor() ??
                  Colors.grey?.textColor(),
            ),
          ),
          background: group.colorString?.toColor() ?? Colors.grey,
