...
class StockChangeProductQuantityViewModel extends ViewModel {
  //Param
  ...
  void init({@required int productTemplateId}) {
    this._productTemplateId = productTemplateId;
    initData();
  }
  ...
  Future<void> _onChangeProduct() async {
    var newModel = await _stockChangeProductQtyApi.getOnChangedProduct(_model);
    if (newModel != null) {
      _model = newModel;
    }
...
//  }
  ...
  Future<bool> save() async {
    ...
    try {
      // Thêm
      var result = await _stockChangeProductQtyApi.insert(_model);
