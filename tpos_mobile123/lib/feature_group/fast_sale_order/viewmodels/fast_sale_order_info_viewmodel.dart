...
class FastSaleOrderInfoViewModel extends ViewModel implements ViewModelBase {
  ...
  Future _submitOrder() async {
    onStateAdd(true, message: "ĝang xác nhận...");
    try {
      final result =
          await _tposApi.fastSaleOrderConfirmOrder(<int>[editOrder.id]);
      if (result.result == true) {
        onDialogMessageAdd(
            OldDialogMessage.flashMessage("ĝã xác nhận hóa đơn"));
      ...
      onDialogMessageAdd(OldDialogMessage.error("Lỗi không xác định!",
  ...
  Future cancelShipOderCommand() async {
    onStateAdd(true, message: "ĝang thực hiện");
    try {
      final result = await _tposApi.fastSaleOrderCancelShip(editOrder.id);
      if (result.error == false) {
        onDialogMessageAdd(
            OldDialogMessage.flashMessage("ĝã hủy vận đơn thành công"));
        await initCommand();
      } else {
        onDialogMessageAdd(
  ...
  Future cancelFastSaleOrderCommand() async {
    ...
    try {
      ...
      if (result.error == false) {
        onDialogMessageAdd(OldDialogMessage.flashMessage("ĝã hủy hóa đơn"));
        await initCommand();
      } else {
        onDialogMessageAdd(
  ...
  Future<void> _resentDeliveryCommandAction() async {
    ...
    try {
      onStateAdd(true, message: "ĝang gửi...");
      await _tposApi.sendFastSaleOrderToShipper(editOrder.id);
      onDialogMessageAdd(
          OldDialogMessage.flashMessage("ĝã gửi lại mã vận đơn"));
    } catch (e, s) {
    ...
    onStateAdd(false, message: "ĝang gửi...");
  }
  ...
  Future<void> callPhone(String phone) async {
    if (phone != null && phone != "") {
      ...
      if (await canLaunch(url)) {
        ...
        onDialogMessageAdd(OldDialogMessage.error("Số điện thoại rỗng!", ""));
      }
    }
  }
  ...
  Future<void> openGoogleMap(String address) async {
    if (address != null && address != "" && address != "null") {
      ...
      onDialogMessageAdd(
          OldDialogMessage.error("ĝịa chỉ rỗng! Không thể mở google map", ""));
    }
  }
