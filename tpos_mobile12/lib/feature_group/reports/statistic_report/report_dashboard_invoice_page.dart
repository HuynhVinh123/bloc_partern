...
class ReportDashboardInvoicePage extends StatefulWidget {
  ...
  @override
  _ReportDashboardInvoicePageState createState() => _ReportDashboardInvoicePageState();
}

class _ReportDashboardInvoicePageState extends State<ReportDashboardInvoicePage> {
  final _limit = 100;
  int _skip = 0;
  ...
  void initState() {
    super.initState();
    bloc.add(OrderFinancialLoaded(limit: _limit,skip: _skip,data: widget.data,isRefund: widget.isRefund));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: _buildAppBar(),
      body:
      BlocUiProvider<ReportDashboardBloc>(
        bloc: bloc,
        listen: (state) {
  ...
  Widget _buildBody() {
    return BlocBuilder<ReportDashboardBloc, ReportDashboardState>(
        buildWhen: (prevState, currState) {
          if (currState is OrderFinancialLoadSuccess || currState is OrderFinancialLoadFailure ) {
            return true;
          }
          return false;
        },
        builder: (context, state) {
          if (state is OrderFinancialLoadSuccess) {
            if (state.invoices.isEmpty) {
              return Container(width: MediaQuery.of(context).size.width,child: EmptyDataPage());
            } else {
        ...
        child: ListTile(
          title: Row(
            children: <Widget>[
              ...
              Text(
                vietnameseCurrencyFormat(widget.isRefund ? item.refundSale ?? 0 : item.amountTotalByOrder ?? 0),
            ...
            children: <Widget>[
              ...
              Row(
                children: <Widget>[
                  ...
                  Text( widget.isRefund ? item.quantityRefund.floor().toString() : item.quantitySale.floor().toString(),
                      style: const TextStyle(
                ...
                child: FlatButton(
                  onPressed: () {
                    ...
                    bloc.add(OrderFinancialLoadMoreLoaded(
                      ...
                      limit: _limit,
                      invoices: items,
                    data: widget.data,isRefund: widget.isRefund));
                  },
