...
class Product {
  ...
  Product.fromJson(Map<String, dynamic> jsonMap) {
    ...
    if (jsonMap["UOMLines"] != null)
      productUOMLines = jsonMap["UOMLines"].map<ProductUOMLine>((map) {
    ...
    if (jsonMap["AttributeValues"] != null)
      attributeValues =
          jsonMap["AttributeValues"].map<ProductAttributeValue>((map) {
        return ProductAttributeValue.fromJson(map);
      }).toList();

    id = jsonMap["Id"];
    variantTeamId = jsonMap["Variant_TeamId"];
    ...
    price = (jsonMap["Price"] ?? 0).toDouble();
  ...
  Map<String, dynamic> toJson({bool removeIfNull = false}) {
    final map = {
      ...
      "UOMLines": uomLines != null
          ? uomLines.map((f) => f?.toJson(removeIfNull)).toList()
          : null,
      "AttributeLines":
          productAttributeLines?.map((f) => f?.toJson(removeIfNull))?.toList(),
      "AttributeValues":
          attributeValues?.map((f) => f?.toJson(removeIfNull))?.toList(),
      "PriceVariant": priceVariant?.toInt(),
      "IsDiscount": isDiscount,
      "NameTemplate": nameTemplate,
      "NameTemplateNoSign": nameTemplateNoSign,
      "Variant_TeamId": variantTeamId,
      "LastUpdated":
          lastUpdated != null ? lastUpdated.toUtc().toIso8601String() : null,
      "DateCreated":
          dateCreated != null ? dateCreated.toUtc().toIso8601String() : null
    };
    // map['TaxesIds'] = <int>[];
