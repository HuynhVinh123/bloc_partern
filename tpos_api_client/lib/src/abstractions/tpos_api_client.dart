...
class TPosApiClient extends TPosApi {
  ...
  Future<T> uploadImage<T>(String path, File file,
      {String contentType, Map<String, dynamic> param}) async {
    try {
      ...
      final Response<T> response = await _dio.post<T>(path, data: formData);
      return response.data;
    } on DioError catch (e) {
      throw _getException(e);
    }
  ...
  Exception _getException(error) {
    if (error is Exception) {
      if (error is DioError) {
          ...
          switch (error.type) {
            ...
            case DioErrorType.RESPONSE:
              switch (error.response.statusCode) {
                ...
                case 504:
                  return TPosApiExceptions.gatewayTimeOut();
                default:
            ...
            case DioErrorType.DEFAULT:
              if (error.error is SocketException) {
                return TPosApiExceptions.socketException(error.error);
              }
              break;
        ...
        return TPosApiExceptions.socketException(error);
      } else {
