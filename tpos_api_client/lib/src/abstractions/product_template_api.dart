...
abstract class ProductTemplateApi {
  ...
  ///Thêm giá biến thể
  Future<ProductAttributeValue> insertVariantPrice(
      {@required int id, @required ProductAttributeValue productAttributeValue});

  ///Lấy biến thể voới id
  Future<Product> getVariant({@required int variantId, String expand = 'AttributeValues'});

  ///Cập nhật thông tin biến thể
  Future<void> updateVariant({@required int id, @required Product product});

  Future<ProductTemplate> onChangeUOM({@required ProductTemplate productTemplate, String expand = 'UOMPO'});

  Future<OdataListResult<ProductUOMLine>> getProductUOMLines({@required int productTemplateId, String expand = 'UOM'});
}
