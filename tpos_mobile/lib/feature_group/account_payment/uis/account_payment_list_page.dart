...
import 'package:tpos_mobile/helpers/messenger_helper.dart' as notify;
...
class _AccountPaymentListPageState extends State<AccountPaymentListPage> {
  ...
  void onUpdateFilter() {
    final compareDateTime = _vm.filterToDate.compareTo(_vm.filterFromDate);
    if (compareDateTime == 1) {
      if (_vm.isFilterByStatus && _vm.filterByStatusString.isEmpty) {
        // Lỗi khi chọn trạng thái để đối tượng  trạng thái trống
        showErrorFilter(S.current.filter_ErrorSelectStatus);
      } else {
        if (_vm.isFilterByTypeAccountPayment && _vm.account.name == null) {
          // Lỗi khi chọn loại phiếu thu mà để đối tượng phiếu thu trống
          showErrorFilter(S.current.filter_ErrorSelectReceiptType);
        } else {
          Navigator.pop(context);
          _vm.isConfirm = true;
          _vm.saveFilter();
        ...
        children: <Widget>[
          ...
          AppFilterPanel(
            ...
            children: <Widget>[
              Container(
                height: 45,
                margin: const EdgeInsets.only(left: 16, right: 12, bottom: 12),
                width: MediaQuery.of(context).size.width,
                child: InkWell(
                  onTap: () {
                    Navigator.push(
        ...
        child: Row(
          children: <Widget>[
            Expanded(
              ...
              padding: const EdgeInsets.only(left: 12),
              child: Text(
                _vm.account?.name != null && _vm.filterByTypeAccountPayment
                    ? _vm.account?.name
                    : S.current.receipts_receiptsType,
            ...
            Visibility(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  _vm.isConfirm = true;
                  _vm.filterByTypeAccountPayment = false;
            ...
            InkWell(
              onTap: () {
                _vm.saveFilter(isOpenDrawer: true);
