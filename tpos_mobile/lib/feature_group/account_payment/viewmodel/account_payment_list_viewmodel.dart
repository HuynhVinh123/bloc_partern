...
class AccountPaymentListViewmodel extends ViewModelBase {
  ...
  /// Biến lưu tạm thông tin checked filter
  bool filterByDate = false;
  bool filterByStatus = false;
  bool filterByTypeAccountPayment = false;
  ...
  Future<void> exportExcel(BuildContext context) async {
    ...
    try {
      if (isFilterByDate) {
        ...
        showNotify(S.current.receipts_pleaseEnterDate);
      }
      setState(false);
    } catch (e, s) {
  ...
  Future<void> getAccountPayments() async {
    ...
    isBusy = true;
    try {
      final result = await _apiClient.getAccountPayments(
          take: limit,
          skip: skip,
          keySearch: _keyword,
  ...
  Future<List<AccountPayment>> loadMoreAccountPayments() async {
    ...
    try {
      skip += limit;
      result = await _apiClient.getAccountPayments(
          take: limit,
          skip: skip,
          keySearch: _keyword,
          accountId: _isFilterByTypeAccountPayment ? _account.id : null,
...
//      showNotify("Xóa dữ liệu thành công!");
  ...
  void saveFilter({bool isOpenDrawer = false}) {
    if (isOpenDrawer) {
      _isFilterByDate = filterByDate;
      _isFilterByStatus = filterByStatus;
      _isFilterByTypeAccountPayment = filterByTypeAccountPayment;
    } else {
      filterByDate = _isFilterByDate;
      filterByStatus = _isFilterByStatus;
      filterByTypeAccountPayment = _isFilterByTypeAccountPayment;
    }
    notifyListeners();
  }
  ...
  void showNotify(String message) {
    _newDialog.showInfo(title: S.current.notification, content: message);
  }
