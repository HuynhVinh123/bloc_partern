...
class SaleOrderAddEditViewModel extends ScopedViewModel {
...
//    _order.dateOrder = order.dateOrder;
  ...
  bool _checkConditionForCreateInvoce() {
    ...
    if (conditions.isNotEmpty) {
      ...
      onDialogMessageAdd(OldDialogMessage.warning(
          "${conditions.join(", ")} ${S.current.purchaseOrder_mustHaveData}"));
      return false;
    }
    return true;
  }
  ...
  Future<bool> saveDraftCommand() async {
    setBusy(true,
        message: "${S.current.saving} ${S.current.draft.toLowerCase()}...");
...
//    assert(_order.state == "draft");
    ...
    try {
      ...
      onDialogMessageAdd(OldDialogMessage.error(
          S.current.purchaseOrder_orderSaveFailed,
        ...
        try {
          ...
          if (confirmResult == true) {
            onDialogMessageAdd(OldDialogMessage.flashMessage(
                "${S.current.purchaseOrder_orderWasConfirmed}. ID: $createdOrderId"));
          } else {
            onDialogMessageAdd(OldDialogMessage.error(
                S.current.purchaseOrder_cannotConfirmOrder,
