...
Widget myFadeAnim({
  ...
  final double beginFade = isFadeIn ? 0.0 : 0.99;
  final double endFade = isFadeIn ? 0.99 : 0;
...
Widget itemFadeAnimation(
  ...
  Animation<double> listTileOpacity;
  length--;
  final double end = (index + 4) / length;
  ...
  listTileOpacity = Tween(begin: 0.0, end: 0.99).animate(
    CurvedAnimation(
      parent: controller,
      curve: Interval(index / length, end > 1 ? 1 : end, curve: Curves.linear),
    ),
  );
  return AnimatedBuilder(
      animation: controller,
      builder: (BuildContext context, Widget trashChild) {
        return FractionalTranslation(
          translation:
              index > 30 ? const Offset(0.0, 0.0) : lisTileTranslation.value,
          child: FadeTransition(
            opacity: index > 30
