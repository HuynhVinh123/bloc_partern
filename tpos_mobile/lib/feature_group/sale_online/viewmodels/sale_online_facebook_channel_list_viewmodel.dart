...
class SaleOnlineFacebookChannelListViewModel extends ScopedViewModel {
  ...
  Future<void> initData() async {
    ...
    try {
      ...
      setBusy(false, isError: true, message: errorMessage);
    }
  }
  ...
  Future<void> addFacebookPageChannel(
    ...
    setBusy(true, message: S.current.adding);
    try {
      final checkResult = await _crmTeamApi.checkFacebookAccount(
        facebookId: page.id,
  ...
  Future<void> _deleteFacebookChannel(CRMTeam crmTeam) async {
    assert(crmTeam != null);
    setBusy(true, message: S.current.deletingDotDotDot);
    try {
      ...
      _log.severe("", e, s);
      _newDialog.showDialog(
        title: S.current.unableToDeleteThisChannel,
        type: AlertDialogType.error,
        content: e.toString(),
      );
    }
  ...
  Future<void> refreshFacebookToken(CRMTeam crmTeam) async {
    ...
    setBusy(true, message: "Đang cập nhật...");
    try {
