...
class ProductTemplateQuickAddEditViewModel extends ScopedViewModel
  ...
  ProductTemplateQuickAddEditViewModel(
    ...
    _productTemplateApi =
        productTemplateApi ?? locator<old.ProductTemplateApi>();
  ...
  //log
  final _log = Logger("ProductQuickAddEditViewModel");
  ...
  List<ProductAttributeValue> get productAttributeValues =>
      _productAttributeValuess;
  ...
  Future<void> addBienThe() async {
    List<ProductAttributeValue> lstAttb = [];
    lstAttb = _productAttributeValuess;

    final ProductAttribute productAttributeLine = ProductAttribute();
  ...
  final BehaviorSubject<List<ProductAttribute>>
      _productAttributeListController = BehaviorSubject();
  Stream<List<ProductAttribute>> get productAttributeListStream =>
      _productAttributeListController.stream;

  final BehaviorSubject<List<ProductAttributeValue>>
      _productAttributeValuesController = BehaviorSubject();
  Stream<List<ProductAttributeValue>> get productAttributeValuesStream =>
      _productAttributeValuesController.stream;

  final BehaviorSubject<ProductAttributeValue> _productAttributeController =
      BehaviorSubject();
  Stream<ProductAttributeValue> get productAttributeStream =>
      _productAttributeController.stream;

  final BehaviorSubject<ProductTemplate> _productController = BehaviorSubject();
  ...
  Future<bool> save(File image) async {
    try {
      ...
      if (product.id == null) {
        ...
        final result = await _productTemplateApi.editProductTemplate(product);
  ...
  Future init() async {
    ...
    if (product.id != null) {
      ...
      final categ = await _tposApi.getProductCategories();
      product.categ = categ?.first;

      product.categId = product.categ?.id;
      final uOM = await _tposApi.getProductUOM();
