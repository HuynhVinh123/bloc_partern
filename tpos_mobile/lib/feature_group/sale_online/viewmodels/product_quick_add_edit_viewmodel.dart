...
class ProductQuickAddEditViewModel extends ScopedViewModel
  ...
  final BehaviorSubject<Product> _productController = BehaviorSubject();
  ...
  Future<bool> loadProduct(int id) async {
    try {
      product = await _productApi.loadProduct(id);
...
//      product.id = 0;
      ...
      if (product.id == null) {
        await _productApi.quickInsertProduct(product);
      } else {
        final result = await _productApi.editProduct(product);
