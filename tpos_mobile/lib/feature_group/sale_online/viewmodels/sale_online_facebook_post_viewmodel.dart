...
class SaleOnlineFacebookPostViewModel extends ScopedViewModel
  ...
  SaleOnlineFacebookPostViewModel(
    ...
    _shopConfig = GetIt.I<ShopConfigService>();
  }
  //log
  ...
  Future initCommand() async {
    try {
      ...
      onDialogMessageAdd(OldDialogMessage.error(
          S.current.saleOnline_DownloadFailed, e.toString()));
    }
    setBusy(false);
  }
  ...
  Future refreshFacebookPost() async {
    ...
    try {
      final facebookPostWithPaging = await _fbApi.getFacebookPostWithPaging(
          ...
          take: _shopConfig.saleFacebookConfig.postCountPerFetchTimes,
      ...
      if (facebookPostWithPaging.error == null) {
        ...
        if (facebookPostWithPaging.error.type == "OAuthException") {
          if (facebookPostWithPaging.error.code == 190) {
            message = "$message \n\n ${S.current.saleOnline_TokenExpired}";
          } else if (facebookPostWithPaging.error.code == 2) {
            message = "$message \n\n S${S.current.saleOnline_ErrorByFb}";
          } else {
            message = "$message \n\n ${S.current.dialog_errorTitle}.";
          }
        }
      ...
      if (_facebookPostsController.isClosed == false)
        _facebookPostsController.addError(S.current.saleOnline_FbError, stack);
    }
  ...
  Future<bool> saveComment(FacebookPost post) async {
    ...
    while (paging != null &&
      ...
      try {
        final result = await _fbApi
            .fetchMoreCommentWithPaging(paging, accessToken, postId: post?.id);
        paging = result.paging;
        lastFetchResults = result.data;
        // ignore: avoid_function_literals_in_foreach_calls
        result.data.forEach(
