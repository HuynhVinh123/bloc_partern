...
///
class NewFacebookPostCommentViewModel extends ScopedViewModel {
  NewFacebookPostCommentViewModel({
    ...
    /// Stream listening
    _realtimeConmmentSubscription = _realTimeCommentSubject
    ...
    _commentsSubscription = _commentsSubject.listen((comment) {
      for (final cm in comment) {
        final model = CommentItemModel(
          facebookComment: cm,
        );
        if (cm.comments != null && cm.comments.isNotEmpty) {
          model.comments = <CommentItemModel>[];
          // ignore: avoid_function_literals_in_foreach_calls
          cm.comments?.forEach(
            (cm) {
    ...
    _dataSubcription = _dataService.dataSubject.stream
        ...
        .listen((data) {
      logger.debug(
          "${S.current.facebook_DetectChange} ${data.value.runtimeType}");
  ...
  /// Filter search comment
  bool _isFilterByCommentHasPhone = false;
  bool _isFilterByCommentHasBuy = false;
  bool _isFilterByCommentHasBuyProduct = false;
  bool _isFilterByStatus = false;

  /// Nhũng biến lưu tạm filter
  bool filterByCommentHasPhone = false;
  bool filterByCommentHasBuy = false;
  bool filterByCommentHasBuyProduct = false;
  bool filterByStatus = false;

  Product _productFilter = Product();

  CommonApi _commonApi;
  Product get productFilter => _productFilter;
  set productFilter(Product value) {
  ...
  bool get isFilterByCommentHasPhone => _isFilterByCommentHasPhone;
  set isFilterByCommentHasPhone(bool value) {
    _isFilterByCommentHasPhone = value;
    notifyListeners();
  }

  bool get isFilterByCommentHasBuy => _isFilterByCommentHasBuy;
  set isFilterByCommentHasBuy(bool value) {
    _isFilterByCommentHasBuy = value;
    notifyListeners();
  }

  bool get isFilterByCommentHasBuyProduct => _isFilterByCommentHasBuyProduct;
  set isFilterByCommentHasBuyProduct(bool value) {
    _isFilterByCommentHasBuyProduct = value;
    notifyListeners();
  }
  ...
  /*END PARAMETER*/
  // Phân  trang
  final int _firstPageSize = 200;
  final int _pageSize = 500;
  int _pageCount = 1;
  bool _isPagingVisible = true;
  int _currentPage = 1;
  int _notReadCommentCount = 0;
  // Khi nhấn xác nhận lọc thì true, sẽ cập nhật lại số lượng filter
  ...
  /*PROPERTY */
  /// Tất cả comment
  final List<CommentItemModel> _comments = <CommentItemModel>[];

  /// Comment livestream
  List<CommentItemModel> _realtimeComments = <CommentItemModel>[];
  ...
  final _realTimeCommentSubject = BehaviorSubject<FacebookComment>();
  final _commentsSubject = BehaviorSubject<List<FacebookComment>>();
  BehaviorSubject<int> realtimeCommentNotifySubject = BehaviorSubject<int>();
  final _searchCommentSubject = BehaviorSubject<String>();
...
/*PUBLIC PROPERTY*/
  ...
  bool get isReverseCommentList {
    if (_shopConfig.saleFacebookConfig.orderCommentBy ==
  ...
  set currentPage(int value) {
    ...
    if (_initLiveStatus == LiveStatus.VOD) {
      ...
      final int skip = (_currentPage - 1) * _pageSize;
      final int take = _pageSize;
      ...
      if (_currentPage == 1) {
        ...
        final int skip =
            _realtimeComments.length + (_currentPage - 2) * _pageSize;
        final int take = _pageSize;
        _viewComments = _comments.skip(skip).take(take).toList();
      }
    }
  ...
  Future setLiveCampaign(LiveCampaign value) async {
    ...
    try {
      // update info
      final LiveCampaign live = _liveCampaign ?? LiveCampaign();
      live.id = value?.id ?? _liveCampaign.id;
      live.facebookLiveId = _facebookPost.id;
      live.facebookUserId = _facebookPost.from?.id;
      live.facebookUserName = _facebookPost.from?.name;
      live.facebookUserAvatar = _facebookPost.picture;
      final facebookPost = TposFacebookPost(
        createdTime: _facebookPost.createdTime,
        facebookId: _facebookPost.id,
        fullPicture: _facebookPost.picture,
        picture: _facebookPost.picture,
        source: _facebookPost.source,
        story: _facebookPost.story,
        from: TposFacebookFrom(
            id: _facebookPost.from?.id,
            name: _facebookPost.from?.name,
            picture: _facebookPost.from?.pictureLink),
        message: _facebookPost.message,
      );
      ...
      _newDialog.showError(title: S.current.error, content: e.toString());
    }
...
/*  METHOD*/
  void init(
      {@required CRMTeam crmTeam, FacebookPost post, String facebookPostId}) {
    assert(crmTeam != null);
  ...
  Future<void> _fetchFacebookComment() async {
    ...
    final fetchResult = await _fbApi.fetchCommentWithPaging(
        accessToken: _crmTeam.userOrPageToken,
        postId: _facebookPost.id,
        limit: _shopConfig.saleFacebookConfig.commentCountPerFetchTimes,
        isNewestOnTop: true);

    _facebookListPaging = fetchResult.paging;
    if (fetchResult.data != null) {
      final commentItemModels =
      ...
      if (_initLiveStatus == LiveStatus.LIVE) {
        final commentToMap = commentItemModels.reversed.toList();
  ...
  Future<void> _fetchAllFacebookComment() async {
    while (_facebookListPaging != null &&
        (_facebookListPaging.next != null ||
            _facebookListPaging.cursors != null)) {
      if (_isCancelAll) break;
      try {
        final result = await _fbApi.fetchMoreCommentWithPaging(
            _facebookListPaging, _crmTeam.userOrPageToken,
            postId: _facebookPost?.id);

        _facebookListPaging = result.paging;
        if (result.data != null) {
          final newComments =
  ...
  Future _fetchRecentComments() async {
    final top50comments = await _fbApi.fetchTopComment(
        top: 50,
        postId: _facebookPost.id,
        accessToken: _crmTeam.userOrPageToken,
  ...
  Future<void> _checkLiveStatus() async {
    ...
    try {
      final LiveVideo liveVideo = await _fbApi.getLiveVideo(
          accessToken: _crmTeam.userOrPageToken,
  ...
  Future<void> _fetchPartner() async {
    ...
    final getResult = await _tposApi.getFacebookPartners(_crmTeam.id);
    _partner.clear();
    getResult.forEach(
      (key, value) {
        _partner.add(
  ...
  Future<void> _fetchOrder() async {
    ...
    final getResult =
        await _tposApi.getOrdersByFacebookPostId(_facebookPost.id);
    if (getResult != null) {
  ...
  void _addCommentToStream(FacebookComment comment,
    ...
    if (checkAny) {
      if (_comments.any((s) => s._facebookComment.id == comment.id)) {
        logger.debug("comment ${comment.id} ${S.current.alreadyExist}");
        return;
      }
    }
  ...
  void _startAutoSaveCommentTimer() {
    ...
    _autoSaveCommentTimer = Timer.periodic(
      Duration(
          minutes: _shopConfig.saleFacebookConfig.autoSaveCommentEveryInMinute),
      (timer) {
        if (_isCancelAll) {
          _autoSaveCommentTimer?.cancel();
          return;
  ...
  //   _realtimeComments = _mapFacebookCommentsToCommentItemModels(comments);
  // }
  ...
  Future<void> _checkReatimeStatus() async {
    ...
    if (_currentLiveStatus == null || _currentLiveStatus == LiveStatus.VOD) {
      //stop timer
      _timer?.cancel();
      _newDialog.showToast(message: "Livestream ${S.current.finished}");
  ...
  Future<void> _mapCommentToOrderAndPartner(
    ...
    for (final cm in comments) {
      final order = _orders.firstWhere(
          ...
          orElse: () => null);
      final partner = _partner.firstWhere(
          (f) => f.facebookAsuid == cm._facebookComment?.from?.id,
          orElse: () => null);
  ...
  Future<void> _listenEventSourceEvent() async {
    ...
    _eventSource = null;
    String filter = "";
    switch (_shopConfig.saleFacebookConfig.fetchCommentRate) {
    ...
    _eventSourceSubscription = _eventSource.events.listen(
      (MessageEvent message) {
        ...
        try {
          final comment = FacebookComment.fromMap(jsonDecode(message.data));
...
/* COMMAND*/
  ...
  Future<void> initCommand() async {
    ...
    try {
      ...
      await _checkLiveStatus();
      notifyListeners();
      setBusy(true, message: S.current.fetchCustomer);
      await Future.wait([
        ...
        _getCommentIds(),
        _fetchTotalProductQuantity(),
      ]);

      setBusy(true, message: S.current.fetchComments);
      ...
      if (_crmTeam.facebookTypeId == "Page") {
        ...
        if (_shopConfig.saleFacebookConfig.fetchCommentDurationSecond > 3) {
          _shopConfig.saleFacebookConfig.fetchCommentDurationSecond = 3;
        }
      }
      ...
      _newDialog
        ...
        content: s.toString(),
          ...
          .then((result) {
        if (result.type == DialogResultType.RETRY) initCommand();
  ...
  Future<void> exportExcel(String postId, BuildContext context) async {
    try {
      setBusy(true);
      final result = await _tposApi.exportExcel(postId);
      if (result != null) {
          ...
          actions: <Widget>[
            ...
            Row(
              ...
              children: <Widget>[
                ...
                FlatButton(
                  child: Text(
                    path != null ? S.current.open : S.current.confirm,
                    style: const TextStyle(fontSize: 16),
                  ),
                  onPressed: () {
                    Navigator.of(context).pop();
                    if (path != null) {
                      ...
                      fetchCommentFacebook(postId, asuid, context);
                    }
                  },
                ),
              ],
  ...
  Future<void> exportExcelByPhone(String postId, BuildContext context) async {
    try {
      setBusy(true);
      final result = await _tposApi.exportExcelByPhone(postId);
      if (result != null) {
        showDialogOpenFileExcel(
            content:
                "${S.current.fileWasSavedInFolder}: $result. ${S.current.doYouWantToOpenFile}",
  ...
  Future<void> fetchCommentFacebook(
      String postId, String asuid, BuildContext context) async {
    try {
      setBusy(true);
      final String videoId = postId.split("_")[1];
      final result = await _tposApi.fetchCommentFacebookByBody(videoId, asuid);
      if (result != null) {
        _newDialog.showToast(
            title: S.current.notification,
            message:
                "${S.current.syncSuccessful} \n Comments: ${result.commentCount} \n ID Update: ${result.updateCount} \n ${S.current.newId}: ${result.newCount}");
      }
      setBusy(false, message: "false");
    } catch (e) {
      _newDialog.showError(title: S.current.error, content: e.toString());
  ...
  void _calculateAutoTime() {
    if (_currentRealtimeMethod == RealtimeMethod.DURATION) {
      ...
      _refreshManualCurrentDurationSecond =
          _shopConfig.saleFacebookConfig.fetchCommentDurationSecond;
      ...
      _commentCountPerMinute = _realtimeComments
          .where((f) => f.facebookComment.createdTime
              .isAfter(DateTime.now().add(const Duration(seconds: -300))))
          .toList()
          .length;
      ...
      print(
          "CM per 5minute: $_commentCountPerMinute. ${S.current.printer} $_refreshManualCurrentDurationSecond");
  ...
  final Queue<CommentItemModel> _createdOrderQuene = Queue<CommentItemModel>();
  final Queue<CommentItemModel> _printOrderQuene = Queue<CommentItemModel>();
  ...
  Future createdOrder(CommentItemModel model) async {
    model.setState(isBusy: true, message: "${S.current.waiting}..");
    _createdOrderQuene.addFirst(model);
    if (_isCreateOrderQueneRunning) return;
    _isCreateOrderQueneRunning = true;
    while (_createdOrderQuene.isNotEmpty) {
      await Future.delayed(const Duration(milliseconds: 200));
      //Create order
      final comment = _createdOrderQuene.last;

      _createOrderAndPrint(comment).catchError((e, s) {
        logger.error("create order failt", e, s);
        _newDialog.showError(
          content: e.toString(),
          title: S.current.createFailed,
        );
  ...
  Future<void> _printOrder(CommentItemModel model) async {
    model.setState(isBusy: true, message: "${S.current.waiting}..");
    _printOrderQuene.addFirst(model);
    if (_isPrintOrderQueneRunning) {
      return;
    }
    ...
    while (_printOrderQuene.isNotEmpty) {
      ...
      final comment = _printOrderQuene.last;
      final order = comment.saleOnlineOrder;
      if (order != null) {
        try {
          comment.setState(isBusy: true, message: S.current.printing);
          int retryCount = 1;
          while (retryCount > 0) {
            try {
...
//              dialogResult.type == DialogResultType.RETRY) break retry;
  ...
  Future<void> _createOrderAndPrint(CommentItemModel comment) async {
    comment.setState(isBusy: true, message: "${S.current.saving}...");
    final SaleOnlineOrder order = SaleOnlineOrder();
    ...
    //checkPartner
    final checkResult = await _tposApi.checkFacebookId(
    ...
    /// Add new phone
    final String phoneFromComment = getPhoneNumber(comment.comment);
    ...
    if (_product != null) {
      ...
      order.details = <SaleOnlineOrderDetail>[];
      order.details.add(
    ...
    order.comments = <TPosFacebookComment>[];
    final bool haveOrder = comment.saleOnlineOrder != null;
    final orderInserted = await _saleOnlineOrderApi.insertFromApp(order);
    // Cập nhật hóa đơn
    ...
    if (!_shopConfig.saleFacebookConfig.allowPrintManyTime) {
      if (comment.isPrinted) {
        exitFunction();
        return;
      }
    }
    if (_shopConfig.saleFacebookConfig.printOnlyOneIfHaveOrder && haveOrder) {
      exitFunction();
      return;
    }
  ...
  Future _onPartnerCheckRequest(String fbAsuid) async {
    final result =
        await _partnerApi.checkPartner(asuid: fbAsuid, crmTeamId: _crmTeam?.id);
    if (result.value != null && result.value.isNotEmpty) {
      final partner = result.value?.first;
      if (partner != null) {
  ...
  void _onPartnerAdded(Partner partner,
      ...
      {bool notify = false,
      String partnerKey}) {
    final partnerComments = _comments
        .where((f) =>
    ...
    if (existsPartner != null) {
      ...
      // Cập nhật địa chỉ mới cho đơn hàng hiện tại
    ...
    partnerComments?.forEach(
      (f) {
        if (isUpdate) {
          ...
          f.saleOnlineOrder?.address = existsPartner.address;
          f.notifyListeners();
        } else {
          f.partnerInfo = existsPartner;
          f.notifyListeners();
    ...
    // Gắn vào reply comment
    final hasReplyCommnets =
        _comments.where((f) => f.comments != null && f.comments.isNotEmpty);

    if (hasReplyCommnets != null && hasReplyCommnets.isNotEmpty) {
      final replyComments = <CommentItemModel>[];
      // ignore: avoid_function_literals_in_foreach_calls
      hasReplyCommnets.forEach(
        (f) {
          replyComments.addAll(
            f.comments?.where(
              (cm) =>
                  cm.facebookComment.from.id == partnerKey ??
                  partner.facebookASids,
            ),
          );
        },
      );
  ...
  Future _onOrderAdded(SaleOnlineOrder order, {bool notifyUi = false}) async {
    ...
    if (existsOrder != null) {
      ...
      //insert
      _orders.add(order);
      _totalProductQuantity += (order.totalQuantity ?? 0)?.toInt();
    }
    // Check partner
    await _onPartnerCheckRequest(order.facebookAsuid);
    ...
    if (hasReplyCommnets != null && hasReplyCommnets.isNotEmpty) {
      ...
      hasReplyCommnets.forEach(
        (f) {
          replyComments.addAll(
            f.comments?.where(
              (cm) => cm.facebookComment.from.id == order.facebookAsuid,
            ),
          );
        },
      );
  ...
  void _onSearchComments(String keyword) {
    if ((keyword == null || keyword.isEmpty) && countFilter() == 0) {
      _searchComments = null;
      notifyListeners();
      return;
    }
    List<CommentItemModel> commentFilters = onSearchFilterComments();

    if (keyword == null || keyword.isEmpty) {
      _searchComments = commentFilters;
    } else {
      commentFilters = commentFilters.isNotEmpty ? commentFilters : _comments;
      final String keywordNoSign =
          StringUtils.removeVietnameseMark(keyword.toLowerCase());
      _searchComments = commentFilters
          .where(
            (f) =>
                StringUtils.removeVietnameseMark(f.facebookName.toLowerCase())
                    .contains(keywordNoSign) ||
                f.facebookComment.from.id.startsWith(keyword) ||
                getPhoneNumber(f.comment).contains(keyword) ||
                (StringUtils.removeVietnameseMark(
                        f.facebookComment.message.toLowerCase())
                    .contains(keywordNoSign.replaceAll("n:", ""))),
          )
          .toList();
    }
  ...
  Future<void> saveComment({bool showError = true}) async {
    ...
    if (fbPost.comment.isEmpty) {
      _newDialog.showToast(message: S.current.noCommentFoundInThisPost);
      return;
    }
    ...
    try {
      ...
      _newDialog.showToast(
        message:
            "${S.current.saved} ${fbPost.comment?.length} ${S.current.comment}",
        type: AlertDialogType.info,
      );
      //Thêm vào list đã lưu
      final commentIds = fbPost.comment.map((f) => f.id).toList();
      _savedCommentIds.addAll(commentIds);
    } catch (e, s) {
      logger.error("save comment", e, s);
      if (showError) {
        _newDialog.showError(content: S.current.saveFailed + e.toString());
      }
    }
  }

  Future<void> _getCommentIds() async {
    final results = await _tposApi.getCommentIdsByPostId(_facebookPost.id);

    if (results != null) {
      _savedCommentIds.addAll(results);
      logger.debug(
          "${S.current.have} ${_savedCommentIds?.length} ${S.current.savedComment}");
    }
  }

  Future<void> hideComment(CommentItemModel comment, bool isHide) async {
    try {
  ...
  Future<void> fetchReplyComment(CommentItemModel comment) async {
    try {
      final fetchCommentReulst = await _fbApi.fetchReplyComment(
          comment.facebookComment.id,
          accessToken: _crmTeam.userOrPageToken,
          // ignore: avoid_bool_literals_in_conditional_expressions
          isNewestOnTop: _shopConfig.saleFacebookConfig.orderCommentBy ==
                  SaleOnlineCommentOrderBy.DATE_CREATE_DESC
              ? false
              : true);
  ...
  void addPartner(Partner partner, CommentItemModel comment) {
  ...
  Future<void> _fetchTotalProductQuantity() async {
    try {
      final result =
          await _tposApi.getProductQuantityByPostId(_facebookPost.id);
      print(result);
      _totalProductQuantity = result ?? 0;
      notifyListeners();
    } catch (e, s) {
      logger.error("", e, s);
    }
  ...
  /// Check chọn status
  void onSelectStatus(int index) {
    partnerStatusReportItems[index].isSelect =
        !partnerStatusReportItems[index].isSelect;

    notifyListeners();
  }

  /// Lấy danh sách trạng thái của khách hàng
  ...
  void resetFilter([bool isReset = false]) {
    _isFilterByCommentHasPhone = false;
    _isFilterByCommentHasBuy = false;
    _isFilterByCommentHasBuyProduct = false;
    _isFilterByStatus = false;
    updateFilterWhenConfirm();
    if (!isReset) {
      _onSearchComments('');
    }
    notifyListeners();
  ...
  // Xác nhận lưu vào biến tam
  void updateFilterWhenConfirm() {
    filterByCommentHasPhone = _isFilterByCommentHasPhone;
    filterByCommentHasBuy = _isFilterByCommentHasBuy;
    filterByCommentHasBuyProduct = _isFilterByCommentHasBuyProduct;
    filterByStatus = _isFilterByStatus;
    notifyListeners();
  }

  // Lưu lại biến  cũ khi open
  void updateFilterWhenOpen() {
    _isFilterByCommentHasPhone = filterByCommentHasPhone;
    _isFilterByCommentHasBuy = filterByCommentHasBuy;
    _isFilterByCommentHasBuyProduct = filterByCommentHasBuyProduct;
    _isFilterByStatus = filterByStatus;
    notifyListeners();
  }

  /// Đếm filter khi nhấn xác nhận
  int count() {
    int _count = 0;

    if (filterByCommentHasPhone) {
      _count++;
    }
    if (filterByCommentHasBuy) {
      _count++;
    }
    if (filterByCommentHasBuyProduct) {
      _count++;
    }
    if (filterByStatus) {
      _count++;
  ...
  // Count filter khi chọn 1 trong các trường hợp
  int countFilter([bool isConfirm = false]) {
    int _count = 0;

    if (_isFilterByCommentHasPhone) {
      _count++;
    }
    if (_isFilterByCommentHasBuy) {
      _count++;
    }
    if (_isFilterByCommentHasBuyProduct) {
      _count++;
    }
  ...
  /// Filter comment theo trạng thái, số điện thoại và có mua hàng
  List<CommentItemModel> onSearchFilterComments() {
    final List<CommentItemModel> _commentSearchs = [];
    ...
    for (final item in _comments) {
      if (countFilter() > 1) {
        if (_isFilterByCommentHasBuy &&
            ...
            item.orderCode != '' &&
            _isFilterByStatus &&
            !_isFilterByCommentHasPhone) {
          final bool isExistStatus =
              partnerStatusReportItems.any((PartnerStatusReportItem element) {
            bool isExist = false;
            if (element.isSelect &&
                item.partnerStatusText == element.statusText) {
              isExist = true;
            } else if (element.isSelect &&
                element.statusText == 'Bình thường') {
              if (item.partnerStatusText != null) {
                isExist = element.statusText == item.partnerStatusText;
              } else {
                isExist = true;
              }
            }
          ...
          if (isExistStatus) {
            _commentSearchs.add(item);
          }
        } else if (_isFilterByCommentHasBuy &&
            item.orderCode != null &&
            item.orderCode != '' &&
            _isFilterByCommentHasPhone &&
            !_isFilterByStatus) {
          final String phoneNumber = getPhoneNumber(item.comment);

          if (phoneNumber != '') {
            _commentSearchs.add(item);
          }
        } else if (_isFilterByStatus &&
            _isFilterByCommentHasPhone &&
            !_isFilterByCommentHasBuy) {
          final bool isExistStatus =
              partnerStatusReportItems.any((PartnerStatusReportItem element) {
            bool isExist = false;
            if (element.isSelect &&
        ...
        } else if (_isFilterByCommentHasPhone) {
          final String phoneNumber = getPhoneNumber(item.comment);
...
class CommentItemModel extends Model {
  CommentItemModel(
    ...
    comments = childs;
    _shopCofig = shopConfig ?? GetIt.I<ShopConfigService>();
  }

  ShopConfigService _shopCofig;
  ...
  String get commentTime {
    return _shopCofig.saleFacebookConfig.showCommentTimeAsTimeAgo
        ? time_ago.format(_facebookComment.createdTime, locale: "vi_short")
        : DateFormat(DateTime.now().day != _facebookComment.createdTime.day
                ? "dd/MM/yyyy HH:mm:ss"
                : "HH:mm:ss")
