...
class GameLuckyWinnerPage extends StatefulWidget {
  const GameLuckyWinnerPage({Key key, this.winners, this.pageAccessToken})
      : super(key: key);
...
class _GameLuckyWinnerPageState extends State<GameLuckyWinnerPage> {
  ...
  void initState() {
    ...
    _luckyWheelWinnerBloc.add(LuckyWheelWinnerInitial(
        players: widget.winners, pageAccessToken: widget.pageAccessToken));
    super.initState();
  }
        ...
        child: Padding(
          ...
          child: Container(
            ...
            child: Container(
                ...
                child: Scaffold(
                    backgroundColor: Colors.transparent, body: _buildBody())),
          ),
        ),
      ),
    );
      ...
      children: [
        ...
        Padding(
          ...
          child: Column(
            children: [
              Expanded(
                  child: BaseBlocListenerUi<LuckyWheelWinnerBloc,
                      LuckyWheelWinnerState>(
                ...
                listener: (BuildContext context, LuckyWheelWinnerState state) {
                  if (state is LuckyWheelWinnerMessageSendSuccess) {
                    final Widget snackBar = getCloseableSnackBar(
                        message: 'Gửi thành công', context: context);
                    ...
                    final Widget snackBar = getCloseableSnackBar(
                        message: 'Chia sẻ thành công', context: context);
                    Scaffold.of(context).showSnackBar(snackBar);
                      ...
                      children: [
                        Flexible(
                          child: Text(
                            player.name,
                            style: const TextStyle(
                                color: Color(0xff008E30), fontSize: 17),
                          ),
                        ),
                        const SizedBox(width: 10),
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
                  ...
                  child: IconButton(
                    ...
                    onPressed: () async {
                      App.showDefaultDialog(
                          ...
                          type: AlertDialogType.error,
                          content:
                              'Hiện tại không thể liên kết được với khách hàng');
                    },
                  ),
                ),
              ),
          ...
          const SizedBox(height: 10),
          ListTile(
            leading: const Icon(FontAwesomeIcons.print,
                color: Color(0xff929daa), size: 23),
            onTap: () {
              _luckyWheelWinnerBloc
                  .add(LuckyWheelWinnerPrinted(player: player));
            },
            title: const Text(
          ...
          ListTile(
            leading: SvgPicture.asset('assets/icon/win.svg',
                color: const Color(0xff929daa)),
            onTap: () async {
              final List<Player> players = await Navigator.push(context,
              ...
              if (players != null) {
                _luckyWheelWinnerBloc
                    .add(LuckyWheelWinnerUpdateLocal(players: players));
              }
            },
            title: const Text(
          ...
          ListTile(
            leading: SvgPicture.asset('assets/icon/share.svg',
                color: const Color(0xff929daa)),
            onTap: () {
            ...
            subtitle: Text(
              ...
              style: TextStyle(
                  color: const Color(0xff2C333A).withOpacity(0.5),
                  fontSize: 13),
            ),
          ),
          ListTile(
            ...
            onTap: () {
              if (widget.pageAccessToken != null) {
                _luckyWheelWinnerBloc
                    .add(LuckyWheelWinnerMessageSent(player: player));
                Navigator.of(context).pop();
              } else {
                App.showDefaultDialog(
                    title: 'Lỗi',
          ...
          ListTile(
            ...
            onTap: () async {
              final bool result = await App.showConfirm(
                  title: 'Xác nhận xóa',
                  content: 'Bạn có muốn xóa các sản phẩm được chọn không?');

              if (result != null && result) {
                _luckyWheelWinnerBloc
                    .add(LuckyWheelWinnerDeleted(player: player));
                Navigator.of(context).pop();
              }
            },
            title: const Text(
