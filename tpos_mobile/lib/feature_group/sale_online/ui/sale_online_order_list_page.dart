        ...
        InkWell(
            onTap: () async {
              ...
              if (barcode != null) {
                ...
                orderViewModel.searchOrderCommand(barcode);
              }
            },
            ...
            Padding(
              ...
              child: Row(
                children: <Widget>[
                  ...
                  Text("${S.current.saleOnlineOrder_Loading}..."),
                ],
              ),
            ),
        ],
  ...
  Widget _buildFilterPanel() {
    return ScopedModelDescendant<SaleOnlineOrderListViewModel>(
      builder: (_, __, ___) => AppFilterDrawerContainer(
        countFilter: orderViewModel.filterCount,
        closeWhenConfirm: true,
        onApply: () {
          orderViewModel.updateCheckFilterApply();
          orderViewModel.initCommand();
        },
        onRefresh: () {
          if (widget.partner != null || widget.postId != null)
            return;
          else {
        ...
        bottomContent: Padding(
          ...
          child: Row(
            children: <Widget>[
              ...
              Text(
                  "${S.current.total}: ${vietnameseCurrencyFormat(orderViewModel.amountTotal)}")
            ],
          ),
        ),
      ),
        ...
        children: <Widget>[
            ...
            child: DropdownButton<String>(
              ...
              items: [
                DropdownMenuItem<String>(
                  ...
                  child: Text(S.current.dateNewToOld),
                ),
                DropdownMenuItem<String>(
                  value: "DateCreated_asc",
                  child: Text(S.current.dateOldToNew),
                ),
                DropdownMenuItem<String>(
                  value: "SessionIndex_asc",
                  child: Text("STT (${S.current.lowToHigh})"),
                ),
                DropdownMenuItem<String>(
                  value: "SessionIndex_desc",
                  child: Text("STT (${S.current.highToLow})"),
                ),
                DropdownMenuItem<String>(
                    value: "Name_asc", child: Text("${S.current.name} (A->Z)")),
              ],
            ),
          ),
          const Expanded(
          ...
          Padding(
            ...
            child: GestureDetector(
              onTap: () {
                orderViewModel.updateCheckFilterOpen();
                _scaffoldKey.currentState.openEndDrawer();
              },
              child: Badge(
                badgeColor: Colors.redAccent,
                badgeContent: Text(
                  "${orderViewModel.filterCountCache ?? 0}",
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              Padding(
                ...
                child: Text(
                  S.current.saleOnlineOrder_WaitProcess,
          ...
          builder: (context, ordersSnapshot) {
            if (ordersSnapshot.hasError) {
              return ListViewDataErrorInfoWidget(
                errorMessage: "${S.current.dialog_errorOccur}! \n" +
                    ...
                    onPressed: () async {
                      if (orderViewModel.selectedIds == null ||
                        ...
                        showWarning(
                            context: context,
                            title:
                                "${S.current.saleOnlineOrder_UnSelectOrder}!",
                            message: S.current.saleOnlineOrder_ChooseOrder);
                        return;
                      }
  ...
  void showDialogProducts(
    ...
    showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            contentPadding: const EdgeInsets.all(0),
            title: Text(S.current.saleOnlineOrder_ProductInOrder),
            content: Container(
              width: 1000,
                  ...
                  title: Row(
                    children: <Widget>[
                      ...
                      if (item.facebookUserId != null)
                        // Mở facebook
                        Tooltip(
                          message: S.current.saleOnlineOrder_OpenFacebook,
                          child: OutlineButton.icon(
                            //textColor: Colors.white,
                            icon: const Icon(
              ...
              ListTile(
                ...
                onTap: () async {
                  final dialogResult = await showQuestion(
                      ...
                      message:
                          "${S.current.purchaseOrder_deleteOrder} ${item.code}");
                      ...
                      Column(
                        ...
                        children: <Widget>[
                          ...
                          RichText(
                            ...
                            text: TextSpan(
                              text: item.telephone ??
                                  "  ${S.current.noPhoneNumber}  ",
                              style:
                                  contentStyle.copyWith(color: Colors.black87),
                              children: [
                                TextSpan(
                                  text:
                                      " | ${item.address ?? "  ${S.current.noAddress} "}",
                          ...
                          InkWell(
                            child: Padding(
                              padding: const EdgeInsets.only(
                                  left: 8, top: 8, right: 8),
                              child: Text(
                                "${NumberFormat("###,###,###").format(item.totalQuantity ?? 0)} ${S.current.product}",
