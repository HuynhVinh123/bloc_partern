...
class NewGameLuckyWheelPage extends StatefulWidget {
  const NewGameLuckyWheelPage({Key key, this.crmTeam, this.postId}) : super(key: key);
...
class _NewGameLuckyWheelPageState extends State<NewGameLuckyWheelPage> {
  ...
  void initState() {
    ...
    _gameLuckyWheelBloc.add(
        GameLuckyWheelLoaded(token: widget.crmTeam.userOrPageToken, crmTeam: widget.crmTeam, postId: widget.postId));
    super.initState();
  }
  ...
  Widget _buildBody() {
    return BaseBlocListenerUi<GameLuckyWheelBloc, GameLuckyWheelState>(
      ...
      errorBuilder: (BuildContext context, GameLuckyWheelState state) {
        ...
        if (state is GameLuckyWheelLoadFailure) {
        ...
        return SafeArea(
          child: Container(
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  SizedBox(height: 150 * (MediaQuery.of(context).size.height / 1000)),
                  LoadStatusWidget(
      ...
      listener: (BuildContext context, GameLuckyWheelState state) async {
        if (state is GameLuckyWheelGameStartError) {
        ...
        } else if (state is GameLuckyWheelWinnerGetError) {
          App.showToast(title: S.current.error, context: context, type: AlertDialogType.error, message: state.error);
        } else if (state is GameLuckyWheelWinnerSaveError) {
          App.showToast(title: S.current.error, context: context, type: AlertDialogType.error, message: state.error);
        } else if (state is GameLuckyWheelWinnerPrintError) {
          App.showToast(title: S.current.error, context: context, type: AlertDialogType.error, message: state.error);
        ...
        } else if (state is GameLuckyWheelWinnerPrintSuccess) {
          App.showToast(
              title: S.current.success,
              context: context,
              type: AlertDialogType.success,
              message: S.current.printWinnerSuccessful);
          ...
          App.showToast(
              ...
              context: context,
              type: AlertDialogType.success,
              message: S.current.successfullyParam(S.current.share));
        } else if (state is GameLuckyWheelWinnerSaveSuccess) {
          App.showToast(
              title: S.current.success,
              context: context,
              type: AlertDialogType.success,
              message: S.current.saveSuccessfulParam(S.current.winner.toLowerCase()));
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  SizedBox(height: 20 * aspectHeight),
                  if (_players != null && _players.isNotEmpty && !_isPlaying) _buildButton() else _buildDisableButton(),
                  SizedBox(height: 10 * aspectHeight),
                  _buildBottom(),
                  SizedBox(height: 10 * aspectHeight),
                ],
                      ...
                      AppButton(
                        ...
                        onPressed: () {
                          _gameLuckyWheelBloc.add(GameLuckyWheelLoaded(
                              token: widget.crmTeam.userOrPageToken, crmTeam: widget.crmTeam, postId: widget.postId));
                        },
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
    ...
    return BaseBlocListenerUi<GameLuckyWheelBloc, GameLuckyWheelState>(
        ...
        builder: (BuildContext context, GameLuckyWheelState state) {
          return Padding(
            ...
            child: SingleChildScrollView(
              child: CongratulationAnimationForeground(
  ...
  Widget _buildWinner(Player player) {
    return Container(
...
class _CircleButton extends StatelessWidget {
  const _CircleButton({Key key, this.icon, this.onPressed, this.backgroundColor}) : super(key: key);
  final Widget icon;
