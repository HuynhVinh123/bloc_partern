    ...
    if (event is ProductUomAddEditStarted) {
      ...
      try {
        if (event.productUom != null) {
          ...
          _productUom = ProductUOM(
              ...
              factorInv: 1,
              uOMType: 'reference',
              showUOMType: S.current.originalUnitOfGroup);
      ...
      try {
        ...
        if (_productUom.name == '' || _productUom.name == null) {
          yield ProductUomAddEditNameError(error: S.current.pleaseEnterParam(S.current.unitName.toLowerCase()));
        } else if (_productUom.productUomCategory == null) {
          yield ProductUomAddEditCategoryError(
              error: S.current.pleaseSelectParam(S.current.unitCategory.toLowerCase()));
        } else {
      ...
      try {
        _productUom = event.productUom;
        await _productUomApi.delete(_productUom.id);
        yield ProductUomAddEditSaveSuccess(productUom: _productUom);
      } catch (e, stack) {
        _logger.e('ProductUomAddEditDeleteError', e, stack);
        yield ProductUomAddEditDeleteError(error: e.toString());
      }
    }
