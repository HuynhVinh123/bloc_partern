...
import 'package:tpos_mobile/widgets/dialog/double_input_dialog.dart';
...
class _ProductUomAddEditPageState extends State<ProductUomAddEditPage> {
  ...
  final Map<String, String> _uomTypes = <String, String>{
    'bigger': S.current.largerOriginalUnit,
    'reference': S.current.originalUnitOfGroup,
    'smaller': S.current.smallerOriginalUnit,
  };
  ...
  Widget _buildBody() {
    return BaseBlocListenerUi<ProductUomAddEditBloc, ProductUomAddEditState>(
      ...
      listener: (BuildContext context, ProductUomAddEditState state) {
        ...
        } else if (state is ProductUomAddEditVerifiedError) {
          App.showToast(
              title: S.current.warning, context: context, message: state.error, type: AlertDialogType.warning);
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(S.of(context).unitCategoryTitle,
                              style: const TextStyle(color: Color(0xff2C333A), fontSize: 17)),
                          ...
                          if (_productUom.productUomCategory != null)
                            ...
                            Text(S.of(context).selectParam(S.of(context).unitCategory.toLowerCase()),
                                style: const TextStyle(color: Color(0xff929DAA), fontSize: 15)),
                        ],
                      ),
                    ),
                          ...
                          child: Material(
                            child: InkWell(
                              onTap: () async {
                                _productUom.showFactor ??= 0;
                                final double number =
                                    await showDoubleInputDialog(context, _productUom.showFactor.toDouble());
                                if (number != null) {
                                  _productUom.showFactor = number;
                                  setState(() {});
                                }
                              },
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    border: Border.all(color: const Color(0xffE9EDF2))),
                                child: Text(
                                  _productUom.showFactor != null ? _productUom.showFactor.toString() : '0',
                                  overflow: TextOverflow.ellipsis,
                          ...
                          child: Material(
                            child: InkWell(
                              onTap: () async {
                                _productUom.rounding ??= 0;
                                final double number =
                                    await showDoubleInputDialog(context, _productUom.rounding.toDouble());
                                if (number != null) {
                                  _productUom.rounding = number;
                                  setState(() {});
                                }
                              },
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    border: Border.all(color: const Color(0xffE9EDF2))),
                                child: Text(
                                  _productUom.rounding != null ? _productUom.rounding.toString() : '0',
                                  overflow: TextOverflow.ellipsis,
                                  style: const TextStyle(fontSize: 17, color: Color(0xff2C333A)),
                                ),
                              ),
