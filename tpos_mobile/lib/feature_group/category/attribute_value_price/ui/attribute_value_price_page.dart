...
class AttributeValuePricePage extends StatefulWidget {
  const AttributeValuePricePage({Key key, this.productTemplate})
      : super(key: key);

  @override
  _AttributeValuePricePageState createState() =>
      _AttributeValuePricePageState();
...
class _AttributeValuePricePageState extends State<AttributeValuePricePage> {
  ...
  void initState() {
    ...
    _attributeValuePriceBloc.add(
        AttributeValuePriceInitial(productTemplate: widget.productTemplate));
    super.initState();
  }
      ...
      actions: [
        IconButton(
            ...
            onPressed: () async {
              ...
              if (result != null) {
                _attributeValuePriceBloc.add(AttributeValuePriceInitial(
  ...
  Widget _buildBody() {
    return BaseBlocListenerUi<AttributeValuePriceBloc,
        AttributeValuePriceState>(
      ...
      builder: (BuildContext context, AttributeValuePriceState state) {
        ...
        return _productAttributeValues.isEmpty
            ? _buildEmptyList()
            : _buildList();
      },
    );
  ...
  Widget _buildList() {
    ...
    for (final ProductAttributeValue productAttributeValue
        in _productAttributeValues) {
      ...
      children: [
        ...
        LoadStatusWidget.empty(
          ...
          content: S.of(context).emptyNotificationParam(
              S.of(context).attributePrice.toLowerCase()),
          action: ClipRRect(
            borderRadius: BorderRadius.circular(24),
            child: AppButton(
              onPressed: () async {
  ...
  Widget _buildItem(
      ProductAttributeValue productAttributeValue, double maxWidth) {
    return Column(
      children: [
        Row(
                          ...
                          child: IconButton(
                            ...
                            onPressed: () async {
                              final ProductAttributeValue result =
                                ...
                                MaterialPageRoute(
                                  builder: (context) =>
                                      AttributeValuePriceDetailsAddEditPage(
                                    productTemplateId:
                              ...
                              if (result != null) {
                                _attributeValuePriceBloc.add(
                                    AttributeValuePriceInitial(
                                        productTemplate:
                                            widget.productTemplate));
                              }
                            },
                          ),
                        ),
