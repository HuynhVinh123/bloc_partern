...
class _PartnerInfoPageState extends State<PartnerInfoPage> {
  ...
  Future<void> _handleEdit() async {
    await context.navigateTo(PartnerAddEditPage(
        ...
        onPartnerStatusChanged: (value) {
          viewModel.updatePartnerStatus2(value);
        }));
          ...
          children: <Widget>[
                ...
                Expanded(
                  child: Padding(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: <Widget>[
                        // Badget trạng thái khách hàng
                        PartnerStatusBadge(
                          partnerId: viewModel.partner?.id,
                          partner: viewModel.partner,
                          statusText: viewModel.partner.statusText,
                          onStatusChanged: (value) async {
                            viewModel.updatePartnerStatus2(value);

                            if (widget.onChangeStatus != null) {
                              widget.onChangeStatus(
                                  viewModel.partner.status,
                                  viewModel.partner.statusStyle,
            ...
            Container(
              ...
              child: ExpansionTile(
                ...
                children: <Widget>[
                  InfoRow(
                    titleString: "${S.current.update}: ",
                    content: Text(
                      viewModel.partner.lastUpdated?.toTimeAgo('vi') ?? "",
                    ...
                    content: Text(
                        viewModel.partner.birthDay != null
                            ? DateFormat("dd/MM/yyyy")
                                .format(viewModel.partner.birthDay.toLocal())
                            : "",
                        style: _infoRowStyle,
                        textAlign: TextAlign.right,
                        maxLines: null),
