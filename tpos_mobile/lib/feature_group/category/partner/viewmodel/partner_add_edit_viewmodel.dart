...
class PartnerViewModel extends PViewModel {
  ...
  void setPartnerStatus({String statusText, String statusStyle}) {
    _status = null;
  ...
  void updateStreetBaseOnAddress() {
    if (_city == null || _district == null || _ward == null) {
      _newDialog.showToast(
        message: S.current.partner_EnterAddress,
  ...
  Future<void> init(
      ...
      Function(Partner) onSaved}) async {
  ...
  void _mapViewModelToPartner() {
    ...
    _partner.companyType = _isPersonal ? 'person' : 'company';
    _partner.propertyProductPricelistId = _priceList?.id;
  }
  ...
  String _validateData() {
    ...
    // Bạn phải chọn đối tác là cá nhân, nhà cung cấp hoặc cả hai
    if (_partner.isCompany == null) {
      return S.current.partner_ChoosePartner;
    }
    ...
    if (_partner.discount.getValueOrDefault() > 100) {
      //Chiết khấu chỉ nằm trong khoảng từ 0 tới 100%
      return S.current.partner_DiscountFrom0To100;
    }

    if (_partner.birthDay != null && !_partner.birthDay.isUtc) {
      return S.current.partner_InvalidTimeFormat;
    }
    return null;
  }
  ...
  Future<PCommandResult> save() async {
    ...
    try {
      if (isAddNew) {
        ...
        // Cập nhật lại UI
        notifyListeners();
        // Đã lưu khách hàng với tên
        _newDialog.showToast(
            message: '${S.current.partner_SaveCustomerWithName} $_name');
      } else {
