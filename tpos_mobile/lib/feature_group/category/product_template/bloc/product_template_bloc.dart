...
class ProductTemplateBloc extends Bloc<ProductTemplateEvent, ProductTemplateState> {
  ProductTemplateBloc(
  ...
  @override
  Stream<ProductTemplateState> mapEventToState(ProductTemplateEvent event) async* {
    if (event is ProductTemplateStarted) {
      ...
      _productTemplateExecutes
          .removeWhere((ProductTemplate productTemplate) => productTemplate.id == event.productTemplate.id);
      yield ProductTemplateLoadSuccess(
          productTemplates: _productTemplates,
          filterTags: _filterTags,
          orderBy: _orderBy,
      ...
      if (_selectedAll) {
        try {
          ...
          await _productTemplateApi
              .deletes(_productTemplates.map((ProductTemplate productTemplate) => productTemplate.id).toList());
          ...
          yield ProductTemplateDeleteFailure(error: e.toString())..copyWith(state);
        }
      } else if (_productTemplateExecutes.isNotEmpty) {
        try {
          yield ProductTemplateBusy()..copyWith(state);
          await _productTemplateApi
              .deletes(_productTemplateExecutes.map((ProductTemplate productTemplate) => productTemplate.id).toList());

          _total -= _productTemplateExecutes.length;
          for (final ProductTemplate productTemplate in _productTemplateExecutes) {
            _productTemplates.removeWhere((ProductTemplate item) => item.id == productTemplate.id);
          }
    ...
    } else if (event is ProductTemplateAssignTag) {
      try {
        await _tagProductTemplateApi.assignTagProductTemplate(
            assignModel: AssignTagProductTemplateModel(productTmplId: event.productTemplate?.id, tags: event.tags));

        event.productTemplate.tags = event.tags;
        yield ProductTemplateAssignTagSuccess(
            productTemplates: _productTemplates,
            filterTags: _filterTags,
            orderBy: _orderBy,
            productPrice: _productPrice,
        ...
        _logger.e('ProductTemplateAssignTagFailure', e, stack);
        yield ProductTemplateAssignTagFailure(error: e.toString())..copyWith(state);
      }
    } else if (event is ProductTemplateOpenFilter) {
      yield ProductTemplateLoadSuccess(
          productTemplates: _productTemplates,
          filterTags: _filterTags,
          orderBy: _orderBy,
  ...
  void updateFilter() {
    _filter = OldOdataFilter(logic: 'and', filters: <OldFilterBase>[
      // Tìm kiếm theo tên, hoặc mã
      if (_keyWord != null && _keyWord != '')
        OldOdataFilter(logic: 'or', filters: <OldOdataFilterItem>[
          OldOdataFilterItem(field: 'Name', operator: 'contains', value: _keyWord),
          OldOdataFilterItem(field: 'NameNoSign', operator: 'contains', value: _keyWord),
          OldOdataFilterItem(field: 'Barcode', operator: 'contains', value: _keyWord),
          OldOdataFilterItem(field: 'DefaultCode', operator: 'contains', value: _keyWord),
        ]),
      ...
      if (_filterProductCategories.isNotEmpty && _isFilterByCategory)
        OldOdataFilter(logic: 'or', filters: <OldOdataFilterItem>[
          ..._filterProductCategories.map(
            (f) => OldOdataFilterItem(field: 'CategId', operator: 'eq', value: f.id),
          )
        ])
  ...
  void updateSort() {
    _sort = OldOdataSortItem(field: "DateCreated", dir: "asc");
  ...
  Stream<ProductTemplateState> mapEventToProductTemplateStarted(ProductTemplateStarted event) async* {
    try {
      ...
      final List<String> tagIdList = _isFilterByTag ? _filterTags.map((Tag tag) => tag.id.toString()).toList() : null;
      ...
      final GetProductTemplateResult result =
          await _productTemplateApi.getList(priceId: priceId, tagIds: tagIds, query: query);
      _total = result.total;
      _productTemplates = result.data;
      final OdataListResult<Tag> odataListResult = await _tagPartnerApi.getTagsByType('producttemplate');
      _tags = odataListResult.value;
      final priceListResult = await _priceListApi.getPriceListAvailable(data: DateTime.now());
  ...
  Stream<ProductTemplateState> mapEventToProductTemplateRefreshed(ProductTemplateRefreshed event) async* {
    try {
      updateFilter();
      final GetProductTemplateQuery query = GetProductTemplateQuery(
        filter: _filter?.toJson(),
  ...
  Stream<ProductTemplateState> mapEventToProductTemplateSearched(ProductTemplateSearched event) async* {
    try {
      _productTemplateExecutes.clear();
      _keyWord = StringUtils.removeVietnameseMark(event.search.toLowerCase() ?? '');
      updateFilter();
      updateSort();
      final GetProductTemplateQuery query = GetProductTemplateQuery(
        filter: _filter?.toJson(),
  ...
  Stream<ProductTemplateState> mapEventToProductTemplateSorted(ProductTemplateSorted event) async* {
    try {
      _productTemplateExecutes.clear();
  ...
  Stream<ProductTemplateState> mapEventToProductTemplateFiltered(ProductTemplateFiltered event) async* {
    try {
  ...
  Stream<ProductTemplateState> mapEventToProductTemplateLoadedMore(ProductTemplateLoadedMore event) async* {
    try {
