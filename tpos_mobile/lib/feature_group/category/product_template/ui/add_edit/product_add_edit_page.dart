...
class ProductTemplateAddEditPage extends StatefulWidget {
  const ProductTemplateAddEditPage({Key key, this.productTemplate}) : super(key: key);

  final ProductTemplate productTemplate;

  @override
  _ProductTemplateAddEditPageState createState() => _ProductTemplateAddEditPageState();
}

class _ProductTemplateAddEditPageState extends State<ProductTemplateAddEditPage> {
  ...
  // final NewDialogService _dialogService = NewDialogService();
  ...
  void initState() {
    ...
    _productTemplateAddEditBloc.add(ProductTemplateAddEditStarted(productTemplate: widget.productTemplate));
  }
  ...
  Widget _buildAppbar() {
    return AppBar(
      ...
      title: Text(
        widget.productTemplate != null ? S.of(context).editProduct : S.of(context).insertProduct,
  ...
  Widget _buildBody() {
    ...
    return BlocProvider<ProductTemplateAddEditBloc>.value(
      ...
      child: BaseBlocListenerUi<ProductTemplateAddEditBloc, ProductTemplateAddEditState>(
        ...
        listener: (BuildContext context, ProductTemplateAddEditState state) {
          ...
          } else if (state is ProductTemplateAddEditNameError) {
            App.showDefaultDialog(
                title: S.of(context).error, context: context, type: AlertDialogType.error, content: state.error);
          }
        },
        errorBuilder: (BuildContext context, ProductTemplateAddEditState state) {
                  ...
                  action: AppButton(
                    onPressed: () {
                      _productTemplateAddEditBloc
                          .add(ProductTemplateAddEditStarted(productTemplate: widget.productTemplate));
                    },
                    width: 180,
              ...
              Column(
                children: [
                  Container(
                    ...
                    child: Row(
                      children: [
                        Expanded(child: _buildHeaderTab(S.of(context).generalInformation, true, 0)),
                        Expanded(child: _buildHeaderTab(S.of(context).variant, false, 1)),
                      ],
                    ),
                  ),
                  Expanded(
    ...
    return InkWell(
      ...
      child: Container(
          ...
          decoration: BoxDecoration(
            ...
            bottom: BorderSide(
              width: 2,
              color: _currentIndex == index ? const Color(0xff28A745) : Colors.transparent,
            ),
          )),
          child: Center(
            child: Text(
              name,
              style: TextStyle(
                  color: _currentIndex == index ? const Color(0xff28A745) : const Color(0xff929DAA), fontSize: 15),
            ),
          )),
    );
  }
