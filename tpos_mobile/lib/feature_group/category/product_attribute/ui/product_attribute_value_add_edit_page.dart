...
class ProductAttributeValueAddEditPage extends StatefulWidget {
  const ProductAttributeValueAddEditPage({Key key, this.productAttributeLine}) : super(key: key);

  @override
  _ProductAttributeValueAddEditPageState createState() => _ProductAttributeValueAddEditPageState();
  final ProductAttributeValue productAttributeLine;
}

class _ProductAttributeValueAddEditPageState extends State<ProductAttributeValueAddEditPage> {
  ...
  void initState() {
    ...
    _productAttributeLineAddEditBloc
        .add(ProductAttributeValueAddEditStarted(productAttributeLine: widget.productAttributeLine));
    super.initState();
  }
  ...
  Widget _buildBody() {
    return BaseBlocListenerUi<ProductAttributeValueAddEditBloc, ProductAttributeValueAddEditState>(
      ...
      listener: (BuildContext context, ProductAttributeValueAddEditState state) {
        if (state is ProductAttributeValueAddEditSaveSuccess) {
        ...
        } else if (state is ProductAttributeValueAddEditLoadSuccess) {
          ...
          builder: (BuildContext context, void Function(void Function()) setState) {
            return InkWell(
              onTap: () async {
                ...
                if (result != null) {
                  result.attributeValues = null;
                  result.productAttribute = null;
                          ...
                          child: Material(
                            child: InkWell(
                              onTap: () async {
                                _productAttributeLine.sequence ??= 0;
                                final double number =
                                    await showNumberInputDialog(context, _productAttributeLine.sequence.toDouble());
                                if (number != null) {
                                  _productAttributeLine.sequence = number.toInt();
                                  setState(() {});
                                }
                              },
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    border: Border.all(color: const Color(0xffE9EDF2))),
                                child: Text(
                                  _productAttributeLine != null ? _productAttributeLine.sequence.toString() : '0',
  ...
  void save() {
    if (_productAttributeLine.productAttribute == null) {
      App.showToast(
          title: S.current.warning,
          context: context,
          message: S.of(context).pleaseSelectParam(S.of(context).attribute.toLowerCase()),
    ...
    if (_nameController.text == '') {
      App.showToast(
          title: S.current.warning,
          context: context,
          message: S.of(context).pleaseEnterParam(S.of(context).attributeValue.toLowerCase()),
          type: AlertDialogType.warning);
      return;
    }
    ...
    if (_productAttributeLine != null) {
      ...
      _productAttributeLineAddEditBloc
          .add(ProductAttributeValueAddEditSaved(productAttributeLine: _productAttributeLine));
    } else {
