...
import 'package:tpos_mobile/app.dart' as app_dialog;
...
import 'package:tpos_mobile/feature_group/fast_sale_order/ui/setting_product_default_page.dart';
...
class _FastSaleOrderQuickCreateFromSaleOnlineOrderPageState
  ...
  final _vm = FastCreateInvoiceWithDefaultProductViewModel();
  ...
  Future<void> initData() async {
    ...
    final bool isExist = await _vm.loadSaleSetting();
    if (isExist && !_vm.isError) {
      await _vm.initData();
    } else if (!isExist && !_vm.isError) {
      showSettingProductDefault();
    }
...
// Thanh toán
  ...
  void showSettingProductDefault() {
    showDialog(
      ...
      builder: (BuildContext contxt) => AlertDialog(
        ...
        title: Text(
          S.current.settingProduct,
          textAlign: TextAlign.center,
        ),
        content: SettingProductDefaultPage(
          vm: _vm,
        ),
        actions: <Widget>[
          // Bỏ qua
          FlatButton(
            ...
            onPressed: () {
              Navigator.pop(context);
              _vm.onEventAdd("GO_BACK", null);
            },
          ),
          FlatButton(
            child: Text(S.current.save.toUpperCase()),
            onPressed: () async {
              if (_vm.defaultProduct.name != null) {
                Navigator.pop(context);
                await _vm.saveSetting();
                await _vm.initData();
              } else {
                app_dialog.App.showToast(
                    title: S.current.warning,
                    context: context,
                    type: AlertDialogType.warning,
                    message: S.current.notifySaveSettingDefaultProduct);
              }
            },
          )
        ],
