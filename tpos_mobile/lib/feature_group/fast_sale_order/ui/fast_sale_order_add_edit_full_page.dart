...
class FastSaleOrderAddEditFullPage extends StatefulWidget {
  const FastSaleOrderAddEditFullPage(
      ...
      this.editOrderCopy})
      : assert(isCopyOrder != null),
        assert((isCopyOrder == true && editOrderCopy != null) ||
            isCopyOrder == false);
  final FastSaleOrder editOrder;
  final List<String> saleOnlineIds;
  final int partnerId;
  final Function(FastSaleOrder) onEditCompleted;
  final bool isCopyOrder;
  final FastSaleOrder editOrderCopy;
...
class _FastSaleOrderAddEditFullPageState
        ...
        if (_vm.carrierService == null) {
          ...
          await showDialog(
            ...
            builder: (context) => AlertDialog(
              // Chưa chọn dịch vụ
              title: Text(S.current.fastSaleOrder_NoServiceSelected),
              // Bạn chưa chọn dịch vụ cho đối tác giao hàng\n- Nhấn [${S.current.fastSaleOrder_ChooseService}] để lựa chọn sau đó hãy nhấn nút xác nhận lại
              content: Text(
                  "${S.current.fastSaleOrder_ChooseServiceForPartner}\n- ${S.current.press} [${S.current.fastSaleOrder_ChooseService}] ${S.current.fastSaleOrder_pressConfirmAgain}"),
              actions: <Widget>[
                FlatButton(
                  child: Text(S.current.fastSaleOrder_ChooseService),
                  onPressed: () {
  ...
  void initState() {
    ...
    _vm.initCommand(
        editOrderCopy: widget.editOrderCopy, isCopy: widget.isCopyOrder);
        ...
        child: ScopedModelDescendant<FastSaleOrderAddEditFullViewModel>(
          builder: (context, child, model) => Scaffold(
            ...
            appBar: AppBar(
              ...
              title: Text(
                  widget.editOrder?.number ?? S.current.saleOnline_NewOrder),
              actions: <Widget>[
                      ...
                      Expanded(
                        child: SizedBox(
                          ...
                          child: DropdownButton<ProductPrice>(
                            hint: Text(S.current.selectPriceList),
                            onChanged: (value) {
                              _vm.priceList = value;
                            },
                        ...
                        child: FlatButton.icon(
                          ...
                          onPressed: () async {
                            if (_vm.partner == null) {
                              showWarning(
                                  title:
                                      "${S.current.fastSaleOrder_NoCustomer}!",
                                  context: context,
                                  message: S.current
                                      .fastSaleOrder_PleaseChoosePartner);
                              return;
                            }
                      ...
                      SizedBox(
                        ...
                        child: FlatButton(
                          ...
                          onPressed: () async {
                            ...
                            // final ScanBarcodeResult result =
                            //     await scanBarcode();
                            final result = await Navigator.push(context,
                                MaterialPageRoute(builder: (context) {
                              return BarcodeScan();
        ...
        child: ListTile(
          ...
          subtitle: _vm.partner == null
              ...
              : Row(
                  ...
                  children: <Widget>[
                    Material(
                      color: _vm.partner.getStatusColor(),
                      ...
                      child: Padding(
                        ...
                        child: Text(
                          ...
                          textAlign: TextAlign.right,
                          style: TextStyle(
                              color: _vm.partner.getStatusColor().textColor()),
                        ),
                      ),
                    ),
                  ],
                      ...
                      Container(
                        child: Column(
                          children: <Widget>[
                            ...
                            // SĐT
                            Text(
                              "${S.current.phoneNumber}: ${_vm.partner.phone ?? ""}",
                              textAlign: TextAlign.center,
                            ),
                            // Địa chỉ
                            Text(
                  ...
                  onPressed: () {
                    if (_vm.partner == null) {
                      showDialog(
                        ...
                        builder: (ctx) {
                          return AlertDialog(
                            title: Text(S.current.fastSaleOrder_NoCustomer),
                            content: Text(S.current
                                .fastSaleOrder_PleaseChoosePartnerToEdit),
                            actions: <Widget>[
                              FlatButton(
                                child: const Text("OK"),
                                onPressed: () {
                        ...
                        await showDialog(
                          ...
                          builder: (context) {
                            return AlertDialog(
                              ...
                              title:
                                  Text("${S.current.fastSaleOrder_noAddress}!"),
                              content: Text(
                                  "${S.current.fastSaleOrder_DeliveryFee}?"),
                              actions: <Widget>[
                                // Quay lại sửa
                                FlatButton(
                                  child: Text(S.current.backToEdit),
                                  onPressed: () {
                                    Navigator.pop(context);
                                    isContinue = false;
                                  },
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                  ...
                                  borderRadius: const BorderRadius.all(
                                      Radius.circular(5))),
                              // Phí giao hàng Thu hộ
                              child: Text(
                                  "${S.current.shippingFee}: ${vietnameseCurrencyFormat(_vm.deliveryPrice)} | ${S.current.cashOnDelivery}: ${vietnameseCurrencyFormat(_vm.cashOnDelivery)}"),
                            ),
                          ),
                        ],
                      ),
                  ...
                  children: <Widget>[
                    Builder(builder: (ctx) {
                      if (_vm.order?.dateInvoice != null) {
                        return Text(
                            "${S.current.invoiceDate} : ${DateFormat("dd/MM/yyyy").format(_vm.order.dateInvoice)} | ${S.current.seller}: ${_vm.user?.name ?? "[${S.current.empty}]"}");
                      } else {
                    ...
                    children: <Widget>[
                      Row(
                        ...
                        children: <Widget>[
                          ...
                          Text(_vm.paymentJournal?.name ??
                              S.current.notSelectedYet)
                        ],
                      ),
                      const SizedBox(
                        height: 3,
          ...
          onTap: () async {
            if (_vm.partner == null) {
              showWarning(
                  ...
                  context: context,
                  message: S.current.fastSaleOrder_PleaseChoosePartner);
              return;
            }
    ...
    return Dismissible(
      ...
      child: ListTile(
            ...
            children: <Widget>[
              ...
              Expanded(
                ...
                child: Builder(
                  builder: (ctxt) {
                    if (item.discount > 0) {
                      return Text(" (${S.current.reduce} ${item.discount}%) ");
        ...
        onLongPress: () {
          showDialog(
            context: context,
            builder: (context) => AlertDialog(
              title: Text(S.current.fastSaleOrder_ChooseAction),
              content: Container(
                width: 400,
                child: Column(
                  mainAxisSize: MainAxisSize.min,
      ...
      confirmDismiss: (direction) async {
        final dialogResult = await showQuestion(
            ...
            message:
                "${S.current.fastPurchase_confirmDelete} ${item.productName}?");
