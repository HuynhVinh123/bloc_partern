...
//Load fast sale order
  ...
  Future _loadFastSaleOrder({bool resetPaging = false}) async {
    ...
    try {
      ...
      _log.severe("", e, s);
      onDialogMessageAdd(
          OldDialogMessage.error("", S.current.network_noInternetConnection));
    } catch (e, s) {
      onIsBusyAdd(false);
  ...
  Future printShipOrderCommand(FastSaleOrder order,
    ...
    try {
      ...
      onDialogMessageAdd(OldDialogMessage.error(
          "${S.current.printFailed} ", e.toString(),
          error: e));
    }
    onStateAdd(false);
  }
  ...
  Future<void> canncelInvoices(List<FastSaleOrder> items) async {
    ...
    try {
      // Đang hủy
      onStateAdd(true, message: "${S.current.canceling}...");
      await _tposApi
  ...
  Future<void> printOrders() async {
    ...
    for (final itm in selectedItem) {
      try {
        // "Đang in
        onStateAdd(true,
            message: "${S.current.printing} ${itm.number ?? "N/A"}");
