...
//      }
  ...
  * */
  ...
  Future<bool> initCommand(
    ...
    assert(isCopy != null);
    onStateAdd(true, message: "${S.current.fastSaleOrder_InitializingData}...");
    ...
    try {
      ...
      if ((_order != null && _order.id != null && _order.id != 0) ||
        ...
        //  Tạo hóa đơn mới
        final OdataResult<FastSaleOrder> getDefaultResult =
            await _fastSaleOrderApi.getFastSaleOrderDefault();
...
//            orderLines = _order.orderLines;
  ...
  Future<void> addOrderLineCommand(Product product) async {
    ...
    Future _addNew() async {
      ...
      try {
        _updateOrderInfo();
        onStateAdd(true, message: "${S.current.update}...");
        final orderLine =
        ...
        onStateAdd(false, message: "${S.current.update}...");
      } catch (e, s) {
        logger.error("", e, s);
        _dialog.showError(
  ...
  Future<void> selectPartnerCommand(Partner partner, [int partnerId]) async {
    ...
    onStateAdd(true, message: "${S.current.checking}...");
    try {
      ...
      onStateAdd(false, message: "${S.current.checking}...");
      onPropertyChanged("");
    } catch (e, s) {
  ...
  Future<void> saveAndConfirmCommand(
    ...
    final dialogResult = await _dialog.showConfirm(
        ...
        yesButtonTitle: S.current.confirm.toUpperCase(),
        noButtonTitle: S.current.cancel.toUpperCase());
    ...
    try {
      ...
      if (result != null && result != 0) {
        ...
        if (confirmResult) {
          ...
          // In phiếu
          if (isPrintShip) {
            onStateAdd(true, message: S.current.printing);
            _printShip(result);
          }
