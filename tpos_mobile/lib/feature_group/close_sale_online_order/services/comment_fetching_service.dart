...
class FacebookCommentFetchingService implements CommentFetchingService {
  FacebookCommentFetchingService(
      {FacebookApi facebookApiService, ShopConfigService shopConfigService}) {
    _facebookApiService = facebookApiService ?? GetIt.I<FacebookApi>();
    _shopConfig = shopConfigService ?? GetIt.I<ShopConfigService>();
    _init();
  }

  /// Facebook Api Service instance
  FacebookApi _facebookApiService;
  ...
  final Map<String, FacebookComment> _commentMaps = <String, FacebookComment>{};
  ...
  final bool _enableFetchByApi = false;

  /// Time delay bettwen two request on page
  int get _delayTimeWhenFetchCommentByApi =>
  ...
  Future<void> checkLiveStatus() async {
    ...
    assert(_facebookPost.type == "video");
    if (_facebookPost.type != FacebookPostType.video.describe()) {
      _liveStatus = null;
    }
  ...
  Future<void> _startFetchComment() async {
    assert(_liveStatus == LiveStatus.LIVE);
...
//          _comments.where((element) => element.fetchByRealtime == false).last;
  ...
  Future<void> _fetchAllComment() async {
    ...
    Future<void> fetchCommentByPaging() async {
      try {
        ...
        for (final comment in fetchResult.data) {
    ...
    Future<void> fetchMoreCommentWithPaging() async {
      ...
      try {
        ...
        fetchResult.data.forEach(_commentSubject.add);
  ...
  void _addCommentToStream(List<FacebookComment> comments) {
    for (final FacebookComment comment in comments) {
  ...
  Future<void> _startFetchCommentByEventSource() async {
    assert(_liveStatus == LiveStatus.LIVE);
    _cancelEventSource();
