...
import 'package:tpos_mobile/feature_group/reports/report_order/report_order_bloc.dart';
import 'package:tpos_mobile/feature_group/reports/report_order/report_order_event.dart';
import 'package:tpos_mobile/feature_group/reports/report_order/report_order_state.dart';
...
class InvoiceReportEmployeePage extends StatefulWidget {
  const InvoiceReportEmployeePage(
      {this.bloc,
        this.dateFrom,
        this.dateTo,
        this.staffId,
  ...
  @override
  _InvoiceReportEmployeePageState createState() =>
      _InvoiceReportEmployeePageState();
}

class _InvoiceReportEmployeePageState extends State<InvoiceReportEmployeePage> {
  final _limit = 10;
  int _skip = 0;
  ...
  void initState() {
    super.initState();
    widget.bloc.add(ReportEmployeeInvoicesLoaded(
        skip: _skip, limit: _limit, staffId: widget.staffId));
  }
      ...
      children: <Widget>[
        ...
        BlocBuilder<ReportOrderBloc, ReportOrderState>(
            ...
            builder: (context, state) {
              if (state is ReportOrderStaffLoading) {
  ...
  Widget _buildList() {
    return BlocBuilder<ReportOrderBloc, ReportOrderState>(
        buildWhen: (prevState, currState) {
          if (currState is InvoiceReportEmployeeTypeSaleLoadSuccess) {
            return true;
          }
          return false;
        },
        cubit: widget.bloc,
        builder: (context, state) {
          if (state is InvoiceReportEmployeeTypeSaleLoadSuccess) {
            return state.invoiceReportEmployees.isEmpty
                ? EmptyDataPage()
                : ListView.builder(
                padding: const EdgeInsets.only(top: 10),
                itemCount: state.invoiceReportEmployees.length,
                itemBuilder: (context, index) => _buildItem(
                    state.invoiceReportEmployees[index],
                    index,
                    state.invoiceReportEmployees));
          }
          return const SizedBox();
        });
  }
  ...
  Widget _buildButtonLoadMore(
    ...
    return BlocBuilder<ReportOrderBloc, ReportOrderState>(
        cubit: widget.bloc,
        builder: (context, state) {
          if (state is ReportOrderPartnerInvoiceLoadMoreLoading) {
            return Center(
              child: SpinKitCircle(
          ...
          return Center(
            child: Container(
                ...
                child: FlatButton(
                  onPressed: () {
                    _skip += _limit;
                    widget.bloc.add(DetailReportCustomerInvoicesLoadMoreLoaded(
                        skip: _skip,
                        limit: _limit,
                        invoices: items,
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          ...
          Text(
            widget.filterDateRange != null
                ? widget.filterDateRange.name == "Tùy chỉnh"
                ? "${DateFormat("dd/MM/yyyy").format(widget.dateFrom)} - ${DateFormat("dd/MM/yyyy").format(widget.dateTo)}"
                : widget.filterDateRange.name
                : "Thời gian",
          ...
          const Icon(Icons.arrow_drop_down, color: const Color(0xFF6B7280))
        ],
      ),
    );
  }
