...
class ReportDeliveryOrderCustomerListPage extends StatefulWidget {
  const ReportDeliveryOrderCustomerListPage({this.bloc, this.partnerId});
  final ReportDeliveryOrderBloc bloc;
  final int partnerId;
  @override
  _ReportDeliveryOrderCustomerListPageState createState() =>
      _ReportDeliveryOrderCustomerListPageState();
}

class _ReportDeliveryOrderCustomerListPageState
    extends State<ReportDeliveryOrderCustomerListPage> {
  final int _limit = 20;
  int _skip = 0;
  ...
  void initState() {
    ...
    widget.bloc.add(DetailDeliveryReportCustomerLoaded(
        limit: _limit, skip: _skip, partnerId: widget.partnerId));
  }
  ...
  Widget _buildBody() {
    return BlocBuilder<ReportDeliveryOrderBloc, ReportDeliveryOrderState>(
        ...
        buildWhen: (prevState, currState) {
          if (currState is DetailDeliveryReportCustomerLoadSuccess) {
            return true;
          }
          return false;
        },
        builder: (context, state) {
          if (state is DetailDeliveryReportCustomerLoadSuccess) {
            return state.invoices.isEmpty
                ? EmptyDataPage()
                : ListView.builder(
                    padding: const EdgeInsets.only(top: 10),
                    itemCount: state.invoices.length ?? 0,
                    itemBuilder: (context, index) => _buildItem(
                        state.invoices[index], index, state.invoices));
          }
          return const SizedBox();
        });
  }
  ...
  Widget _buildItem(ReportDeliveryOrderDetail item, int index,
      List<ReportDeliveryOrderDetail> detailReportCustomers) {
    return item.userName == "detailTemp"
        ? _buildButtonLoadMore(index, detailReportCustomers)
        : Container(
  ...
  Widget _buildButtonLoadMore(
      int index, List<ReportDeliveryOrderDetail> detailReportCustomers) {
    return BlocBuilder<ReportDeliveryOrderBloc, ReportDeliveryOrderState>(
        cubit: widget.bloc,
        builder: (context, state) {
          ...
          return Center(
            child: Container(
                ...
                child: FlatButton(
                  onPressed: () {
                    _skip += _limit;
                    widget.bloc.add(DetailDeliveryReportCustomerLoadMoreLoaded(
                        limit: _limit,
                        skip: _skip,
                        invoices: detailReportCustomers,
