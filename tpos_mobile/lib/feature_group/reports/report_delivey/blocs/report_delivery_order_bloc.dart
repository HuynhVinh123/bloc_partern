...
class ReportDeliveryOrderBloc
  ...
  Stream<ReportDeliveryOrderState> mapEventToState(
    ...
    if (event is ReportDeliveryFastSaleOrderLoaded) {
      ...
      /// Load more danh sách hóa đơn cho nhân viên
      yield DeliveryReportStaffLoadMoreLoading();
      yield* _getDetailDeliveryReportStaffLoadMore(event);
    } else if (event is DetailDeliveryReportCustomerLoaded) {
      /// Load  hóa đơn cho khách hàng
      yield DeliveryReportStaffLoading();
      yield* _getDetailDeliveryReportCustomer(event);
    } else if (event is DetailDeliveryReportCustomerLoadMoreLoaded) {
      /// Load more hóa đơn cho khách hàng
      yield DeliveryReportStaffLoadMoreLoading();
      yield* _getDetailDeliveryReportCustomerLoadMore(event);
    }
  }
  ...
  Stream<ReportDeliveryOrderState> _getDetailDeliveryReportCustomer(
      DetailDeliveryReportCustomerLoaded event) async* {
    try {
      final List<ReportDeliveryOrderDetail> invoices =
          await _apiClient.getInvoicesDeliveryReportCustomer(
              take: event.limit,
              skip: event.skip,
              dateFrom: _filterReportDelivery.dateFrom,
              dateTo: _filterReportDelivery.dateTo,
      ...
      if (invoices.length == event.limit) {
        invoices.add(tempDetailReportDeliveryStaff);
      }

      yield DetailDeliveryReportCustomerLoadSuccess(invoices: invoices);
    } catch (e) {
      yield ReportDeliveryFastSaleOrderLoadFailure(
          title: S.current.notification, content: e.toString());
    }
  ...
  Stream<ReportDeliveryOrderState> _getDetailDeliveryReportCustomerLoadMore(
      DetailDeliveryReportCustomerLoadMoreLoaded event) async* {
    try {
      event.invoices.removeWhere((element) =>
          element.userName == tempDetailReportDeliveryStaff.userName);
      ...
      event.invoices.addAll(detailDeliveryReportLoadMores);

      yield DetailDeliveryReportCustomerLoadSuccess(invoices: event.invoices);
    } catch (e) {
      yield ReportDeliveryFastSaleOrderLoadFailure(
          title: S.current.notification, content: e.toString());
    }
