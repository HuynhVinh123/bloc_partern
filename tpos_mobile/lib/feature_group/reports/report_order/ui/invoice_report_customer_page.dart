...
class InvoiceReportCustomerPage extends StatefulWidget {
  const InvoiceReportCustomerPage(
      {this.bloc,
      this.dateFrom,
      this.dateTo,
      this.partnerId,
  ...
  @override
  _InvoiceReportCustomerPageState createState() =>
      _InvoiceReportCustomerPageState();
}

class _InvoiceReportCustomerPageState extends State<InvoiceReportCustomerPage> {
  final _limit = 100;
  int _skip = 0;
  ...
  void initState() {
    super.initState();
    widget.bloc.add(DetailReportCustomerInvoicesLoaded(
        skip: _skip, limit: _limit, partnerId: widget.partnerId));
  }
      ...
      children: <Widget>[
        ...
        BlocBuilder<ReportOrderBloc, ReportOrderState>(
            ...
            builder: (context, state) {
              if (state is ReportOrderPartnerInvoiceLoading) {
  ...
  Widget _buildList() {
    return BlocBuilder<ReportOrderBloc, ReportOrderState>(
        buildWhen: (prevState, currState) {
          if (currState is InvoiceReportCustomerTypeSaleLoadSuccess) {
            return true;
          }
          return false;
        },
        cubit: widget.bloc,
        builder: (context, state) {
          if (state is InvoiceReportCustomerTypeSaleLoadSuccess) {
            return state.invoiceReportCustomers.isEmpty
                ? EmptyDataPage()
                : ListView.builder(
                    padding: const EdgeInsets.only(top: 10),
                    itemCount: state.invoiceReportCustomers.length,
                    itemBuilder: (context, index) => _buildItem(
                        state.invoiceReportCustomers[index],
                        index,
                        state.invoiceReportCustomers));
          }
          return const SizedBox();
        });
  }
