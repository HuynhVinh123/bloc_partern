...
class ReportOrderBloc extends Bloc<ReportOrderEvent, ReportOrderState> {
  ReportOrderBloc({ReportOrderApi reportSaleOrderApi})
  ...
  Stream<ReportOrderState> _getInvoicesReportCustomerLoadMore(
    ...
    try {
      ...
      if (event.partnerId != null) {
        invoiceMores = await _apiClient.getInvoicesDetailReportCustomer(
            top: event.limit,
            skip: event.skip,
            dateFrom: _dateFrom,
            dateTo: _dateTo,
      ...
      if (invoiceMores.length == event.limit) {
        invoiceMores.add(tempDetailReportCustomerTypeSale);
      }
