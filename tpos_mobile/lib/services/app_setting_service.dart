...
class AppSettingService implements ISettingService {
  ...
  DateTime get shopAccessTokenExpire {
    if (_shopAccessTokenExpire == null) {
      final String valueString = prefs.getString(shopAccessTokendExpireKey);
  ...
  SaleOnlineCommentOrderBy get saleOnlineCommentDefaultOrderByOnLive {
    ...
    final String value =
        prefs.getString(saleOnlineCommentDefaultOrderByOnLiveKey);
  ...
  SaleOnlineCommentOrderBy get saleOnlineCommentDefaultOrderByOnPost {
    ...
    final String value =
        prefs.getString(saleOnlineCommentDefaultOrderByOnPostKey);
  ...
  CommentRate get saleOnlineFetchCommentOnRealtimeRate {
    ...
    final String value =
        prefs.getString(saleOnlineFetchCommentOnRealtimeRateKey);
    if (value != null) {
  ...
  List<PrinterDevice> get printers {
    if (_printers == null) {
      try {
        final json = prefs.getString(SETTING_PRINTER_LIST_KEY);
        if (json != null && json != "") {
  ...
  set printers(List<PrinterDevice> value) {
    if (value != null) {
      ...
      final json = value.map((f) => f.toJson()).toList();
      final jsonHasEncode = jsonEncode(json);
  ...
  bool get isShopTokenValid {
    if (shopUrl != null &&
        ...
        shopAccessToken != "" &&
        shopAccessTokenExpire != null &&
        shopAccessTokenExpire.isAfter(
          DateTime.now(),
        )) {
      return true;
    } else {
  ...
  List<PrintTemplate> getSuportPrintTemplateByPrinterType(
      String print, String printerType) {
    final Map printProfile = supportPrinterProfiles[print];
