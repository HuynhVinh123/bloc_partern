...
/*  CÁC HÀM IN MỚI*/
  Future<void> printFacebookShared(FacebookShareCount data);
...
class PosPrintService implements PrintService {
  ...
  PrintShipConfig _getPrintShipOtherConfig(PrinterConfig printShipConfig) {
    ...
    if (others != null) {
      ...
      shipConfig.isHideCompanyEmail = others
              .firstWhere((f) => f.key == "config.hide_company_email",
                  orElse: () => null)
              ?.value ??
          false;
      shipConfig.iHideCompanyPhoneNumber = others
              .firstWhere((f) => f.key == "config.hide_company_phone_number",
                  orElse: () => null)
              ?.value ??
          false;
      shipConfig.isShowProductNoteOnShippingBill = others
              .firstWhere(
                  (f) => f.key == "config.show_product_note_on_shipping_bill",
                  orElse: () => null)
              ?.value ??
          false;
      shipConfig.isOnlyPrintBarcode = others
              .firstWhere((f) => f.key == "config.only_print_barcode",
                  orElse: () => null)
              ?.value ??
          false;
      shipConfig.isShowQrCodeTrackingRefSort = others
              .firstWhere(
                  (f) => f.key == "config.show_QrCode_tracking_ref_sort",
                  orElse: () => null)
              ?.value ??
          false;
      shipConfig.isShowProductInfo = others
              .firstWhere((f) => f.key == "config.show_product_info",
                  orElse: () => null)
              ?.value ??
          false;
      return shipConfig;
    ...
    printConfig.others.forEach((f) {
      switch (f.keyConfig) {
        ...
        case Config.hide_company_phone_number:
          ...
          break;
        case Config.show_seri:
          webPrintConfig.showSeri = f.value;
          break;
        case Config.show_number_red:
          webPrintConfig.showNumberRed = f.value;
          break;
        case Config.show_date_red:
          webPrintConfig.showDateRed = f.value;
          break;
        case Config.show_QrCode:
          webPrintConfig.showQrCode = f.value;
          break;
        case Config.hide_tracking_area:
          webPrintConfig.hideTrackingArea = f.value;
          break;
        case Config.show_QrCode_tracking_ref_sort:
          webPrintConfig.showQrCodeTrackingRefSort = f.value;
          break;
        case Config.chang_trackingRef_and_trackingrefsort:
          webPrintConfig.changTrackingRefAndTrackingRefSort = f.value;
          break;
        case Config.hide_company_email:
          webPrintConfig.hideCompanyEmail = f.value;
          break;
      }
    });
  ...
  Future<void> printOrder(
    ...
    // Lấy máy in
    final printerDevice = _shopConfig.printerConfig.printerDevices.firstWhere(
        (f) => f.name == _shopConfig.printerConfig.invoicePrinterName,
        orElse: () => null);

    if (printerDevice == null) {
      throw Exception(
          "Không tìm thấy máy in được cấu hình: ${_shopConfig.printerConfig.invoicePrinterName}. Vui lòng chọn lại trong Cài đặt-> Máy in -> Hóa đơn");
    }
    ...
    if (printerDevice.type == "preview") {
      ...
      if (invoice != null) {
        invoice = invoice
            .replaceAll(
                'href="/Content', 'href="${_secureConfig.shopUrl}/Content')
            .replaceAll('/Web', '${_secureConfig.shopUrl}/Web');
  ...
  Future<void> printShip(
    ...
    if (order.carrierDeliveryType == "OkieLa") {
      final link = await _fastSaleOrderApi.printOkialaShip(order.id);
    ...
    // Đọc cấu hình
    final PrinterDevice printerDevice = _shopConfig.printerConfig.printerDevices
        .firstWhere((f) => f.name == _shopConfig.printerConfig.shipPrinterName);
    final printShipConfig = await _getPrintShipConfig();

    // In preview nếu máy in là preview và in qua lan
    if (_shopConfig.printerConfig.shipPrinterName == "Xem và in") {
...
//        invoiceAmount: order.amountTotal,
        ...
        // Ghi chú cấu hình trong phiếu ship
        shipNote: temDeliveryNote,
        // Ghi chú giao hàng cần bổ sung thêm Ghi chú sản phẩm
        ...
        qrTrackingRefSort: order.trackingRefSort,
        staff: order.userName,
        depositAmount: order.amountDeposit,
        productQuantity: order.productQuantity,
        productName: order.orderLines
            ?.map((f) => "${f.productUOMQty?.toInt()}  ${f.nameTemplate}")
            ?.join(", "),
    ...
    if (!_shopConfig.printerConfig.printShipShowDepositAmount) {
      dataPrint.depositAmount = null;
    }

    if (!_shopConfig.printerConfig.printShipShowProductQuantity) {
    ...
    if (printerDevice.type == "tpos_printer") {
      ...
      await _tposDesktop.printShip(
        ...
        data: dataPrint,
        size: _shopConfig.printerConfig.shipPaperSize,
  ...
  Future<void> _printShipViaLanImage({
    ...
    try {
      ...
      if (config.isHideCompanyEmail) data.companyEmail = null;
      if (config.isHideCod) data.cashOnDeliveryPrice = null;

      // Ẩn tiền hàng + tiền ship
      ...
      ///TODO(): Bổ sung cấu hình máy in phiếu ship qua Lan, Mẫu 80 Image font đẹp
      // Thiếu Ẩn email công ty
      if (config.isHideCompanyEmail) {
        data.companyEmail = null;
      }

      // Thiếu Ẩn số điện thoại công ty
      if (config.iHideCompanyPhoneNumber) {
        data.companyPhone = null;
      }

      // Thiếu Hiện ghi chú sản phẩm phiếu in đối tác
      if (!config.isShowProductNoteOnShippingBill) {
        data.orderLines?.forEach((f) {
          f.note = null;
        });
      }

      // Thiếu Ẩn tất cả thông tin, chỉ in mã vận đơn
      if (config.isOnlyPrintBarcode) {
        ...
        data.receiverCityName = null;
        data.receiverDictrictName = null;
        data.receiverWardName = null;
        data.invoiceAmount = null;
        data.deliveryPrice = null;
        data.cashOnDeliveryPrice = null;
        data.carrierService = null;
        data.shipWeight = null;
        data.shipNote = null;
        data.content = null;
        data.note = null;
        ...
        data.receiverAddressFull = null;
        data.orderLines = null;
      }

      // Thiếu hiện QrCode mã chia
      if (!config.isShowQrCodeTrackingRefSort) {
        data.qrTrackingRefSort = null;
      }

      // Thiếu hiện thông tin sản phẩm
      if (!config.isShowProductInfo) {
        data.orderLines = null;
      }

      // Chỉ hiện thông tin người mua/bán
      if (config.isHideInfoShip) {
        data.invoiceNumber = null;
        data.invoiceDate = null;
        data.staff = null;
        data.cashOnDeliveryPrice = null;
  ...
  Future<void> _printShipViaLanEsc(
    ...
    if (config.isOnlyPrintBarcode) {
      ...
      /// Số lượng sản phẩm
      if (_shopConfig.printerConfig.printShipShowProductQuantity == true &&
          data.productQuantity != null) {
        form.textLine(
      ...
      /// Tiền cọc
      if (_shopConfig.printerConfig.printShipShowDepositAmount == true &&
          data.depositAmount != null) {
        form.textLine(
      ...
      // Thiếu hiện thông tin sản phẩm
      if (config.isShowProductInfo) {
        String productInfo = "Thông tin SP: ";
      ...
      if (data.trackingRefSort != null &&
          config.isHideTrackingRefSort == false) {
        if (config.isShowQrCodeTrackingRefSort) {
          form.qrCode(data.trackingRefSort,
              qrSize: QRSize.Size8, align: PosTextAlign.center);
          form.textLine(
  ...
  Future<void> _printShipViaLan(
    ...
    if (_shopConfig.printerConfig.shipPaperSize == "BILL80-IMAGE") {
  ...
  Future<void> printSaleOnlineTag(
    ...
    Future<String> _tryGetPartnerCode(int partnerId) async {
      ...
      try {
        final _partner = await _partnerApi.getById(partnerId);
        return _partner?.ref;
      } catch (e, s) {
        _log.severe("print", e, s);
        return null;
    ...
    // read header setting
    if (_shopConfig.saleFacebookConfig.printCustomHeader) {
      printData.header =
          _shopConfig.saleFacebookConfig.customHeader; // Header tùy chỉnh
    } else {
    ...
    if (note == null || note == "") {
      note = "";
      if (_shopConfig.saleFacebookConfig.printAllOrderNote || isPrintNode) {
        note = order.note;
      }
// In địa chỉ
      if (_shopConfig.saleFacebookConfig.printAddress == true) {
...
// In Comment
      if (_shopConfig.saleFacebookConfig.printComment == true &&
          _shopConfig.saleFacebookConfig.printAllOrderNote == false) {
      ...
      if (_shopConfig.saleFacebookConfig.printPartnerNote) {
        if (note?.isNotEmpty == true) {
    ...
    if (printerDevice.type == 'tpos_printer') {
      ...
      if (_shopConfig.saleFacebookConfig.printPaperSize == "BILL80-IMAGE") {
        ...
        try {
          ...
          settingContents.forEach((content) async {
            ...
            switch (content.name) {
              case "header":
                if (_shopConfig.saleFacebookConfig.customHeader
                    .isNotNullOrEmpty()) {
                  form.textLine(
                    _shopConfig.saleFacebookConfig.customHeader,
                    styles: textStyle,
                  );
                }
                break;
  ...
  Future<void> printSaleOnlineTagImage80mm({
    ...
    try {
      ...
      final printerProfile = await ProfileManager()
          .getProfile(printerDevice.profileName ?? "default");
      final PosForm imageForm =
          PosForm(size: PaperSize.bill80, profile: printerProfile)
            ..imageForm(image, ignore: printerDevice.isImageRasterPrint)
            ..imageRaster(image, ignore: !printerDevice.isImageRasterPrint)
            ..emptyLines(5)
            ..cut();
  ...
  Future<void> _printOrderViaLanImage({
    ...
    try {
      ...
      printData.companyEmail = company.email;
      ...
      printData.qrCodeTrackingRefSort = order.trackingRefSort;
      ...
      printData.showQRtrackingRefSort =
          webPrintConfig.showQrCodeTrackingRefSort;
      ...
      printData.changeTrackingRefAndTrackingRefSort =
          webPrintConfig.changTrackingRefAndTrackingRefSort;
      ...
      if (webPrintConfig.hideCompanyEmail) printData.companyEmail = null;
      if (webPrintConfig.hideCompanyPhoneNumber) printData.companyPhone = null;
      if (webPrintConfig.showQrCodeTrackingRefSort)
        printData.qrCodeTrackingRefSort = null;
  ...
  Future<void> _printOrderViaLanEsc({
    ...
    if (!webPrintConfig.hideCompanyEmail) {
      form.textLine(company?.email, styles: style);
    }
...
//Kí tên
    ...
    if (webPrintConfig.showTrackingRefSoft &&
      ...
      if (webPrintConfig.showQrCodeTrackingRefSort) {
        form.qrCode(order.trackingRefSort,
            qrSize: QRSize.Size8, align: PosTextAlign.center);
        form.textLine("${order.trackingRefSort}",
            styles: style.copyWith(bold: false, align: PosTextAlign.center));
      } else {
        form.barcode(
  ...
  Future<void> _printOrderLan(PrinterDevice printerDevice,
    ...
    if (_shopConfig.printerConfig.invoicePaperSize == "BILL80-IMAGE") {
  ...
  Future<void> _printOrderTPosPrinter(PrinterDevice printerDevice,
    ...
    printerConfig.others.forEach((f) {
      switch (f.keyConfig) {
        ...
        ///TODO(): Bổ sung cấu hình máy in phiếu bán hàng
        // Thiếu cấu hình
        // Hiện ngày HĐ đỏ
        case Config.show_date_red:
          webPrintConfig.showDateRed = f.value;
        ...
        case Config.show_QrCode_tracking_ref_sort:
          webPrintConfig.showQrCodeTrackingRefSort = f.value;
          break;
        // Đổi vị trí mã vận đơn và mã chia
        case Config.chang_trackingRef_and_trackingrefsort:
          webPrintConfig.changTrackingRefAndTrackingRefSort = f.value;
          break;
        case Config.show_seri:
  ...
  Future<void> printGame(
    ...
    if (printerDevice.type == 'esc_pos') {
      ...
      if (_shopConfig.saleFacebookConfig.printPaperSize == "BILL80-IMAGE") {
        ...
        await escPrinter.printFormSplit(form,
            bufferSize:
                printerDevice.overideSetting ? printerDevice.packetSize : null,
            sleepTime:
                printerDevice.overideSetting ? printerDevice.delayTimeMs : null,
...
//        .firstWhere((f) => f.type == "esc_pos", orElse: () => null);
    final PrinterDevice settingPrinter = _shopConfig.printerConfig.printerDevices
...
//        .firstWhere((f) => f.type == "esc_pos", orElse: () => null);
  ...
  @override
  Future<void> printFacebookShared(FacebookShareCount fbShare) async {
    try {
      final printerDevice = _shopConfig.printerConfig.printerDevices.firstWhere(
          (element) =>
              element.name == _shopConfig.printerConfig.saleOnlinePrinterName,
      ...
      final printData =
          PrintSaleOnlineData(name: fbShare.name, uid: fbShare.facebookUid);
      ...
      if (printerDevice.type == 'esc_pos') {
        ...
        if (_shopConfig.saleFacebookConfig.printPaperSize == "BILL80-IMAGE") {
          ...
          imagePrinter.printTextLn(fbShare.name, style: canvasStyles);
          if (fbShare.facebookUid != null)
            imagePrinter.printTextLn(fbShare.facebookUid, style: canvasStyles);
          ...
          const textStyle = PosStyles(
              ...
              isRemoveMark: true);
          form.setLineSpacing(100);
          form.textLine(fbShare.name, styles: textStyle);
          form.textLine(fbShare.facebookUid, styles: textStyle);
          form.resetLineSpacing();
          form.emptyLines(4);
          form.cut();

          await escPrinter.printFormSplit(form,
              bufferSize: printerDevice.overideSetting
                  ? printerDevice.packetSize
                  : null,
              sleepTime: printerDevice.overideSetting
                  ? printerDevice.delayTimeMs
                  : null,
              delayBeforeDisconnect: printerDevice.overideSetting
                  ? printerDevice.delayBeforeDisconnectMs
                  : null);
