...
class TposApiService implements ITposApiService {
  ...
  Future<List<DistrictAddress>> getDistrictAddress(String cityCode) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        print("Response body: ${response.body}");
        final jsonMap = jsonDecode(response.body);
        final districtAddress = jsonMap as List;
  ...
  Future<Map<String, GetFacebookPartnerResult>> getFacebookPartners(
    ...
    if (response.statusCode == 200) {
      final Map<String, GetFacebookPartnerResult> results =
          <String, GetFacebookPartnerResult>{};
  ...
  Future<SaleOnlineOrder> insertSaleOnlineOrderFromApp(SaleOnlineOrder order,
    ...
    final String body = jsonEncode(temp);
  ...
  Future<List<ProductAttribute>> getProductAttribute(productId) async {
    final param = {
...
//    paramMap["\$Expand"] = "Details";
  ...
  Future<TPosApiResult<bool>> fastSaleOrderCancelShip(int orderId) async {
    ...
    if (response.statusCode.toString().startsWith("2")) {
      ...
      if (json["error"] != null) {
        final odataError = OdataError.fromJson(json["error"]);
        return TPosApiResult(
  ...
  Future<TPosApiResult<bool>> cancelSaleOrder(int orderId) async {
    ...
    if (response.statusCode.toString().startsWith("2")) {
      ...
      //Return error and message
      final Map resultMap = jsonDecode(response.body);
      final error = OdataError.fromJson(resultMap["error"]);
      print(error.message);
      return TPosApiResult(
  ...
  Future<TPosApiResult<int>> accountPaymentCreatePost(
    ...
    if (response.statusCode.toString().startsWith("2")) {
      ...
      final erorr = OdataError.fromJson(jsonDecode(response.body));
      return TPosApiResult(
          error: true,
  ...
  Future<TPosApiResult<List<AccountJournal>>>
    ...
    if (response.statusCode == 200) {
      final results = (jsonDecode(response.body)["value"] as List)
          ...
          .toList();
      return TPosApiResult(error: false, result: results);
    } else {
      return TPosApiResult(
          error: true,
          result: null,
  ...
  Future<OdataResult<List<PaymentInfoContent>>> getPaymentInfoContent(
    ...
    //if (response == 200) {
    final Map jsonMap = jsonDecode(response.body);
    final lists = (jsonMap["value"] as List)
  ...
  Future<String> getFastSaleOrderPrintDataAsHtml(
    ...
    final response = await _tposClient.httpGet(path: url, param: param);
    if (response.statusCode == 200)
  ...
  Future<void> addMailTemplate(MailTemplate template) async {
    final Map bodyMap = template.toJson(true);
    final response = await _tposClient.httpPost(
      path: "/odata/MailTemplate",
      body: jsonEncode(bodyMap),
    );
  ...
  Future<String> getFacebookPSUID(
      {@required String asuid, @required String pageId}) async {
  ...
  Future<void> sendFastSaleOrderToShipper(int fastSaleOrderId) async {
    final bodyMap = {"id": fastSaleOrderId};
    final reponse = await _tposClient.httpPost(
...
//          JsonEncoder.withIndent('   ').convert(jsonDecode(response.body)));
      final Account account =
          Account.fromJson(jsonDecode(response.body)["Account"]);
  ...
  Future<FastPurchaseOrder> actionInvoiceDraftFPO(
    ...
    final Map<String, dynamic> body = item.toJson();
  ...
  Future<List<FacebookWinner>> getFacebookWinner() async {
    List<FacebookWinner> results;
    final response = await _tposClient.httpGet(
      path: "/odata/SaleOnline_Facebook_Post/ODataService.GetWinners",
    );
    if (response.statusCode == 200) {
      final jsonMap = jsonDecode(response.body);
      final facebookWinner = jsonMap["value"] as List;
...
//    }..removeWhere((key, value) => value == null);
      ...
      body: jsonEncode(
        {
          "data": {
            "email": email,
            "password": password,
            "host": _secureConfig.shopUrl?.replaceAll("https://", ""),
            "apiKey": apiKey,
          },
          "provider": provider,
          "config": {"apiKey": ""}
  ...
  Future<List<PartnerDetailReport>> getPartnerDetailReports({
    ...
    if (response.statusCode == 200) {
      ...
      for (final partnerDetailReport in _partnerDetailReports) {
  ...
  Future<List<PartnerStaffDetailReport>> getPartnerStaffDetailReports(
    ...
    if (response.statusCode == 200) {
      ...
      for (final partnerDetailReport in _partnerStaffDetailReports) {
        partnerDetailReport.totalItem = map["Total"];
      }
  ...
  Future<List<SupplierReport>> getSupplierReports(
    ...
    if (response.statusCode == 200) {
      ...
      for (final supplierReport in _supplierReports) {
  ...
  Future<List<PartnerDetailReport>> getSupplierDetailReports(
    ...
    if (response.statusCode == 200) {
      ...
      for (final partnerDetailReport in _supplierDetailReports) {
        partnerDetailReport.totalItem = map["Total"];
      }
  ...
  @override
  Future<List<ProductAttributeValue>> getProductAttributeSearch() async {
    ...
    if (response.statusCode == 200) {
      ...
      _productAttributes = (map["value"] as List)
          .map((val) => ProductAttributeValue.fromJson(val))
          .toList();
  ...
  @override
  Future<List<ProductAttributeValue>> getProductAttributeValueSearch() async {
    List<ProductAttributeValue> _productAttributes = [];
  ...
  Future<String> exportExcel(String postId) async {
    ...
    if (status == PermissionStatus.granted) {
      ...
      final response = await dio.download(
          "${_secureConfig.shopUrl}/facebook/exportcommentstoexcel?postid=$postId",
...
//      FileUtils.mkdir([dirloc]);
      final response = await dio.download(
          "${_secureConfig.shopUrl}/facebook/exportcommentstoexcel?postid=$postId&isPhone=true",
  ...
  Future<FetchComment> fetchCommentFacebookByBody(
    ...
    final ISettingService _setting = locator<ISettingService>();
    final shopConfig = GetIt.I<ShopConfigService>();
    final printer = _shopConfig.printerConfig.printerDevices.firstWhere(
        (element) =>
            element.name == shopConfig.printerConfig.saleOnlinePrinterName,
        orElse: () => null);
    ...
    final response = await http.post(
      "http://${printer.ip}:${printer.port}/api/facebook/fetchcomments",
      body: body,
      headers: {
        "Content-Type": 'application/json',
      },
    ...
    _log.info(
        "RESPONSE FROM TPOS API POST: http://${printer.ip}:${printer.port}/api/facebook/fetchcomments:");
    _log.info(response.statusCode);
    _log.info(response.body);
    final mapResponse = json.decode(response.body);

    if (mapResponse["Success"] == null) {
      final result = await _tposClient.httpPost(
          path: "/api/facebook/fetchcommentscallback", body: response.body);
      if (result.statusCode == 200) {
        final mapData = json.decode(result.body);
        if (mapData["success"]) {
    ...
    final List<int> bytes = response.body.toString().codeUnits;
    jsonDecode(utf8.decode(bytes));
    throw Exception(jsonDecode(utf8.decode(bytes))["Message"]);
  }
...
//      FileUtils.mkdir([dirloc]);
      ...
      campaignName = campaignName.replaceAll("\|", "-");
      print(
          "${_secureConfig.shopUrl}/SaleOnline_Order/ExportFile?campaignId=$campaignId&sort=date");
      final response = await dio.download(
          "${_secureConfig.shopUrl}/SaleOnline_Order/ExportFile?campaignId=$campaignId&sort=date",
  ...
  Future<BaseModelSaleQuotation> getSaleQuotations(
    ...
    const String path = "/SaleQuotation/List";
    final Map<String, dynamic> body = {
      "take": take,
      "skip": skip,
      "page": page,
    ...
    if (response.statusCode == 200) {
      ...
      final BaseModelSaleQuotation baseModelSaleQuotation =
          BaseModelSaleQuotation(
  ...
  Future<void> updateSaleQuotation(
    ...
    final String body = json.encode(saleQuotationDetail.toJson());
