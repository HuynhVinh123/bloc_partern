...
typedef _HeaderBuilder<T> = Widget Function(BuildContext context, bool isSelected, T object);
  ...
  Widget _buildTabItem(int index, T object) {
    return GestureDetector(
      onTap: () {
        if (_selectedIndex != index) {
          ...
          if (widget.onSelectedChange != null) {
            widget.onSelectedChange(object);
          }
        }
      },
      child: widget.headerBuilder == null
          ? Container(
              ...
              decoration: BoxDecoration(
                ...
                bottom: BorderSide(
                  width: 2,
                  color:
                      _selectedIndex == index ? widget.underLineColor ?? const Color(0xff28A745) : Colors.transparent,
                ),
              )),
              child: Text(
                object.toString(),
          ...
          : widget.headerBuilder(context, _selectedIndex == index, object),
    );
  }

  ///Di chuyển tới tab dựa vào [index]
  Future<void> _scrollToIndex(int index) async {
    await _controller.scrollToIndex(index, preferPosition: AutoScrollPosition.middle);
