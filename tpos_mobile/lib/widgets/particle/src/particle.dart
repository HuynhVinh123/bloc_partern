...
class ParticleSystem extends ChangeNotifier {
  ...
  void update() {
    ...
    if (_particleSystemStatus == ParticleSystemStatus.stopped && _particles.isEmpty) {
  ...
  bool _isOutsideOfBorder(Offset particleLocation) {
    ...
    return false;
    // (globalParticlePosition.dy >= _bottomBorder) ||
  ...
  List<Particle> _generateParticles({int number = 1}) {
    return List<Particle>.generate(
        number,
        (i) => Particle(_generateParticleForce(), _randomColor(), _randomSize(), _gravity, _particleDrag,
            offset: _offset));
  }
  ...
  vmath.Vector2 _generateParticleForce() {
    ...
    final double num = (min + random.nextInt(max - min)) * random.nextDouble();
    const y = 0.0;
    final x = blastRadius * cos(blastDirection) + num;
...
class Particle {
  Particle(vmath.Vector2 startUpForce, Color color, Size size, double gravity, double particleDrag, {double offset = 0})
      : _startUpForce = startUpForce,
        ...
        // _size = size,
        _pathShape = createPath(size, offset: offset),
  ...
  static Path createPath(Size size, {double offset = 0}) {
    final pathShape = Path();
    pathShape.moveTo(0, offset);
    pathShape.lineTo(-size.width, offset);
    pathShape.lineTo(-size.width, offset - size.height);
    pathShape.lineTo(0, offset - size.height);
    pathShape.close();
    return pathShape;
  }
