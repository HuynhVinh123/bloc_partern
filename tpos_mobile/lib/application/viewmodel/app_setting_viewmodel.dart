...
class ApplicationSettingViewModel extends ViewModel {
  ...
  Future printSaleOnlineTest() async {
    try {
      ...
      onDialogMessageAdd(OldDialogMessage.flashMessage(
          S.current.setting_settingSaleOnlineCheckPrint));
    } catch (ex) {
  ...
  Future resetSaleOnlineSessionNumber() async {
    try {
      ...
      onDialogMessageAdd(OldDialogMessage.flashMessage(
          S.current.setting_settingSaleOnlineResetNumberOfSlip));
    } catch (ex, s) {
      onDialogMessageAdd(OldDialogMessage.error(
          S.current.setting_settingSaleOnlineCannotResetNumberOfSlip,
          ex.toString()));
      logger.error(
          "reset ${S.current.setting_settingSaleOnlineNumberOfSlip}", ex, s);
    }
  }
}
