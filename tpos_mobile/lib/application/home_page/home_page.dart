...
class _HomePageState extends State<HomePage> {
    ...
    return WillPopScope(
      onWillPop: () async {
        final bool result = await App.showConfirm(
          ...
          content: 'Nhấn "Xác nhận" để thoát khỏi ứng dụng',
        );
        ...
        child: BlocListener<NotificationBloc, NotificationState>(
          listener: (context, state) {
            if (state is NotificationLoadSuccess) {
              if (state.notificationResult.popup != null &&
                  _popupRead == false) {
                Navigator.push(
                  context,
                  MaterialPageRoute(
  ...
  Widget _buildContent() {
    switch (_currentIndex) {
      case 0:
        if (GetIt.instance<ShopConfigService>().homePageStyle == "style1") {
