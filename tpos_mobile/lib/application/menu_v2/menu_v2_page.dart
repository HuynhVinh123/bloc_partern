        ...
        children: <Widget>[
          ...
          InkWell(
            child: Container(
              ...
              height: 40,
              child: CircleAvatar(
                backgroundColor: Color(0xff75CC57),
...
class __FavoriteMenuSwipperState extends State<_FavoriteMenuSwipper> {
  ...
  @override
  Widget build(BuildContext context) {
    final menu = _config.favoriteMenu;

    List<FavoriteMenuItem> items = <FavoriteMenuItem>[];
    ...
    final int max = context.isTablet ? 16 : 8;

    for (var i = 0; i < items.length; i += max) {
      final end = (i + max < items.length) ? i + max : items.length;
      final group = items.sublist(i, end);
      pages.add(FavoriteMenuContainer(
        items: group,
      ));
    ...
    final Widget swiper = Swiper.children(
      outer: true,
      loop: false,
