...
import 'package:tpos_mobile/feature_group/settings/configuration/ui/configuration_sync_page.dart';
...
import 'feature_group/conversation/page/conversations_page.dart';
...
import 'feature_group/tpage/channels/ui/facebook_channel_list_page.dart';
...
class TPosApp extends StatefulWidget {
  const TPosApp({Key key}) : super(key: key);
...
class _TPosAppState extends State<TPosApp> {
          ...
          BlocListener<ApplicationBloc, ApplicationState>(
            listener: (context, ApplicationState applicationState) async {
              if (applicationState is ApplicationResetSuccess) {
                ...
                context.read<LanguageBloc>().add(LanguageLoaded());
                setState(() {});
              }
            },
          ),
  ...
  Future<void> checkSDKVersion() async {
...
Map<String, WidgetBuilder> _buildRoute() => {
      ...
      AppRoute.printersSetting: (context) => const SettingPrinterListPage(),
      ...
      // Report
      AppRoute.inventoryReport: (context) => const ReportStockOverviewPage(),
      ...
      AppRoute.partnerGroup: (context) =>
          const PartnerCategoriesPage(isSearchMode: false),
      ...
      AppRoute.channels_tpage: (context) => FacebookChannelListPage(),
      ...
      AppRoute.configurationSync: (context) => MobileConfigSyncPage(),
    };
...
class TPosAppPreview extends StatefulWidget {
  const TPosAppPreview({Key key}) : super(key: key);

  @override
  _TPosAppPreviewState createState() => _TPosAppPreviewState();
}

class _TPosAppPreviewState extends State<TPosAppPreview> {
  final ApplicationBloc _applicationBloc = ApplicationBloc();
