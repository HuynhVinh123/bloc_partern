...
class PosPrintService implements PrintService {
  ...
  PrintShipConfig _getPrintShipOtherConfig(PrinterConfig printShipConfig) {
    final PrintShipConfig shipConfig = new PrintShipConfig();
  ...
  Future<void> printShip(
    ...
    if (order.carrierDeliveryType == "OkieLa") {
      var link = await _fastSaleOrderApi.printOkielaShip(order.id);
      ...
      if (Platform.isIOS) {
        ...
        if (!download) {
          ...
          String fileName = "${tempDir.path}/${link.split("/").last}";
          var res = await Dio().download(link, fileName);
          File file = File.fromUri(Uri(path: fileName));

          Printing.layoutPdf(
            onLayout: (PdfPageFormat format) async => file.readAsBytesSync(),
            format: PdfPageFormat(252, 252),
          );
        } else {
          urlLauch(link);
        }
...
//        invoiceAmount: order.amountTotal,
  ...
  Future<void> _printShipViaLanEsc(
    ...
    final PrinterProfile printerProfile =
        await ProfileManager().getProfile(settingPrinter.profileName);
    PosForm form = PosForm(size: PaperSize.bill80, profile: printerProfile);
  ...
  Future<void> printSaleOnlineTag(
    ...
    Future<String> _tryGetPartnerCode(int partnerId) async {
      ...
      try {
        var parnter = await _partnerApi.getById(partnerId);
        return parnter?.ref;
      } catch (e, s) {
        _log.severe("print", e, s);
        return null;
...
//        );
        ...
        try {
          ...
          PosForm form = PosForm(size: PaperSize.bill80, profile: profile);
          form.reset();
          form.setLineSpacing(110);
  ...
  Future<void> printSaleOnlineTagImage80mm({
    ...
    try {
      ...
      var printerProfile = await ProfileManager()
  ...
  Future<void> _printOrderViaLanEsc({
    ...
    double revenue;
    if (webPrintConfig.showRevenue) {
      var getRevenueResult = await _partnerApi.getRevenueById(order.partnerId);
      revenue = getRevenueResult.revenueTotal;
    }
    ...
    if (_companyCache.imageUrl.isNotNullOrEmpty()) {
      try {
        ...
        var shipForm = PosForm(size: PaperSize.bill80, profile: printerProfile);
        shipForm.logo(
            imageData: ByteData.view(bytes.buffer), width: 300, height: 170);
        shipForm.feed(1);

        var printer = EscPrinter(
            profile: printerProfile,
            host: printerDevice.ip,
            port: printerDevice.port,
            timeout: Duration(seconds: 5));
...
//Kí tên
  ...
  Future<void> _printOrderLan(PrinterDevice printerDevice,
    ...
    var webPrintConfig = _getPrintInvoiceOtherConfig(printerConfig);
  ...
  Future<void> _printOrderTPosPrinter(PrinterDevice printerDevice,
    ...
    var company = await _companyApi.getById(order.companyId);
    var partner = await _partnerApi.getById(order.partnerId);
  ...
  Future<void> printGame(
    ...
    if (_setting.printMethod == PrintSaleOnlineMethod.LanPrinter) {
      ...
      if (_setting.saleOnlineSize == "BILL80-IMAGE") {
        print("print Image");
        CanvasPrinter imagePrinter = CanvasPrinter();
        CanvasStyles canvasStyles = CanvasStyles(
          align: TextAlign.center,
          bold: true,
          fontSize: 45,
        );
    ...
    if (settingPrinter.type == "tpos_printer") {
      ...
      if (_setting.posOrderPrintSize == "BILL80-IMAGE") {
        ...
        try {
          ...
          var columnStyle = PosStyles(align: PosTextAlign.left);
...
//        .firstWhere((f) => f.type == "esc_pos", orElse: () => null);
    ...
    CanvasPrinter canvasPrinter = new CanvasPrinter();
    try {
      CanvasStyles normalStyle = CanvasStyles();
      ...
      if (data.isLogo && data.imageLogo.isNotNullOrEmpty()) {
        Uint8List bytes = await networkImageToByte('${data.imageLogo}');
      ...
      var columnStyle = CanvasStyles(align: TextAlign.left);
      ...
      var printer = EscPrinter(
          ...
          port: settingPrinter.port,
          timeout: Duration(seconds: 5));
      var form = PosForm(size: PaperSize.bill80, profile: profile);
