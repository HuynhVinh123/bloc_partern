...
class AlertApiImpl implements AlertApi {
  ...
  @override
  Future<List<OrderFinancial>> getOrderFinancial({int limit,int skip,String overViewValue}) async {
    ...
    var response = await _apiClient.httpGet("/odata/Financial/ODataService.GetOrderFinancial",parameters: body);
    invoices = (response["value"] as List).map((e) => OrderFinancial.fromJson(e)).toList();
    return invoices;
  }

  @override
  Future<List<OrderFinancial>> getOrderFinancialRefund({int limit, int skip, String overViewValue}) async {
    List<OrderFinancial> invoices = [];
    Map<String,dynamic> body = {"OverViewValue" : overViewValue ?? ""};
    body["TimeSearch"] = "";
    body["Type"] = "refund";
