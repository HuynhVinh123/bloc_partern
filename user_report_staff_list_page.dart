...
import 'package:tpos_mobile/feature_group/sale_online/blocs/user_report_staff/user_report_staff_bloc.dart';
...
class UserReportStaffListPage extends StatefulWidget {
  @override
  _UserReportStaffListPageState createState() => _UserReportStaffListPageState();
}
...
class _UserReportStaffListPageState extends State<UserReportStaffListPage> {
  ...
  Widget build(BuildContext context) {
    return BlocUiProvider<UserReportStaffBloc>(
      ...
      child: Scaffold(appBar: AppBar(title: Text("Danh sách người bán"),), body: _buildBody(context)),
    );
  }
  ...
  Widget _buildListItem(UserReportStaffLoadSuccess state,BuildContext context) {
    return ListView.separated(
        itemBuilder: (context, index) => _buildItem(state.userReportStaffs[index],context),
        separatorBuilder: (context, index) => const Divider(),
        itemCount: state.userReportStaffs.length);
  }

  Widget _buildItem(UserReportStaffOrder item,BuildContext context) {
    return InkWell(
      onTap: (){
