...
class ReportSaleOrder {
  ...
  ReportSaleOrder.fromJson(Map<String, dynamic> json) {
    ...
    odataCount = json['@odata.count'];
    if (json['value'] != null) {
      value = <ReportSaleOrderInfo>[];
      json['value'].forEach((v) {
        value.add(ReportSaleOrderInfo.fromJson(v));
      });
    }
  }
}
...
class ReportSaleOrderLine {
  String odataType;
  int id;
  dynamic productUOSQty;
  int productUOMId;
  dynamic invoiceUOMId;
  int invoiceQty;
  int sequence;
  double priceUnit;
  int productUOMQty;
