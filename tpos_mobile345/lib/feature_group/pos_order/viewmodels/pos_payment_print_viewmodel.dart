...
//    payment.creationDate = _carts[0].time;
    ...
    if (_lines.isEmpty) {
      _dialog.showNotify(
          ...
          message: S.current.posOfSale_noProduct);
    } else if (multiPayments[multiPayments.length - 1].amountDebt > 0) {
      _dialog.showNotify(
          title: S.current.notification,
          message: "Bạn chưa thanh toán đủ tiền");
    } else {
      if (checkInvoice) {
        if (payment.partnerId != null) {
          ...
          _dialog.showError(
              title: S.current.notification,
              content: S.current.posOfSale_partnerIsNotEmpty);
        }
      } else {
        handlePayment(context, _carts[0], _posConfigs[0].ifacePrintAuto);
      }
      ...
      if (result) {
        ...
        if (autoPrint) {
          try {
            ...
            _dialog.showError(
                title: S.current.posOfSale_printError, content: e.toString());
          }
        }
        ...
        if (checkInvoice) {
          ...
          _dialog.showError(error: S.current.posOfSale_paymentFailed);
        }
      }
      setState(false);
