...
class ReportStockIncurredBloc extends Bloc<ReportStockIncurredEvent, ReportStockIncurredState> {
  ReportStockIncurredBloc({DialogService dialogService, ReportStockApi reportStockApi}) : super(ReportStockIncurredLoading()){
  ...
  @override
  Stream<ReportStockIncurredState> mapEventToState(ReportStockIncurredEvent event) async* {
      if(event is ReportStockIncurredLoaded){
        ...
        yield* _getReportStockIncurred(ReportStockIncurredLoaded(skip: event.skip,limit: event.limit));
      }else if(event is ReportStockIncurredLocalFilterSaved){
        _stockFilter = event.stockFilter;
      }
  }

  Stream<ReportStockIncurredState>  _getReportStockIncurred(ReportStockIncurredLoaded event) async* {
    try {
      final ReportStock reportStockIncurred = await _apiClient.getReportStockExt(
      ...
      yield ReportStockIncurredLoadSuccess(reportStockIncurred: reportStockIncurred);
    } catch (e) {
  ...
  Stream<ReportStockIncurredState> _getReportStockIncurredLoadMore(
    ...
    try {
      ...
      final ReportStock reportStockLoadMores = await _apiClient.getReportStockExt(
          fromDate: _stockFilter.dateFrom,
          toDate: _stockFilter.dateTo,
          limit: event.limit,
          skip: event.skip,
      ...
      yield ReportStockIncurredLoadSuccess(reportStockIncurred: event.reportStockIncurred);
    } catch (e) {
      yield ReportStockIncurredLoadFailure(
          title: S.current.notification, content: e.toString());
    }
