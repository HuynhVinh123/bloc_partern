...
class RtmpCameraConnector(val context: Context, val useOpenGL: Boolean, val isPortrait: Boolean, val connectChecker: ConnectCheckerRtmp, val urls: ArrayList<String>) :
    ...
    /// Tạo luống phát liveStream thứ 2
    private lateinit var srsFlvMuxers: ArrayList<SrsFlvMuxer>
    ...
    private fun quantity() {
        srsFlvMuxers = ArrayList<SrsFlvMuxer>()
        if (urls != null) {
            for (i: Int in 0 until urls.size) {
                srsFlvMuxers.add(SrsFlvMuxer(this))
            }
        }
    }
    ...
    fun setProfileIop(profileIop: Byte) {
        ...
        for (i: Int in 0 until srsFlvMuxers.size) {
            srsFlvMuxers[i].setProfileIop(profileIop)
        }
    }
    ...
    fun prepareVideo(width: Int, height: Int, fps: Int, bitrate: Int, hardwareRotation: Boolean,
        ...
        /// Kiểm tra nếu video encoder == null : Khởi tạo cho VideoEncoder
        // Nếu != null : thì prepare videl(Dành cho việc thêm page trong lúc live)
     ...
     *
     * @param bitrate AAC in kb. (64 128kbps)
    ...
    fun startMultiStream(urls: ArrayList<String>?) {
        if (isStreaming) {
            return;
        }
        isStreaming = true
        startMultiStreamRtp(urls)
    }
    ...
    fun runStartEncoderWhenAddPage(){
        startEncoders()
    }
    ...
    // Xử lý stop các cấu hình về video và hình ảnh
    fun closeConfigVideoAndAudio(){
        videoEncoder!!.stop()
        audioEncoder.stop()
    ...
    /// Hàm tự connect lại nếu bị hỏng luồng
    fun reTry(delay: Long, reason: String): Boolean {
        var result : Boolean = false
        if(srsFlvMuxers != null){
            for (i: Int in 0 until srsFlvMuxers.size) {
                result = shouldRetry(reason, srsFlvMuxers[i])
                if (result) {
                    resetVideoEncoder()
                    reConnect(delay,srsFlvMuxers[i])
                }
            }
        }
    ...
    fun setAuthorization(user: String, password: String) {
        ...
        for (i: Int in 0 until srsFlvMuxers.size) {
            srsFlvMuxers[i].setAuthorization(user, password)
        }
    ...
    fun prepareAudioRtp(isStereo: Boolean, sampleRate: Int) {
        for (i: Int in 0 until srsFlvMuxers.size) {
            srsFlvMuxers[i].setIsStereo(isStereo)
            srsFlvMuxers[i].setSampleRate(sampleRate)
        }
    }

    fun prepareAudioRtpWhenAddPage(isStereo: Boolean, sampleRate: Int) {
        srsFlvMuxer.setIsStereo(isStereo)
        srsFlvMuxer.setSampleRate(sampleRate)

//        for (i: Int in countUrlOld until srsFlvMuxers.size) {
//            srsFlvMuxers[i].setIsStereo(isStereo)
//            srsFlvMuxers[i].setSampleRate(sampleRate)
//        }
    }
    ...
    fun startMultiStreamRtp(urls: ArrayList<String>?) {
        if (this.urls != null) {
            for (index: Int in 0 until this.urls.size) {
                if (videoEncoder!!.rotation == 90 || videoEncoder!!.rotation == 270) {
                    srsFlvMuxers[index].setVideoResolution(videoEncoder!!.height, videoEncoder!!.width)
                } else {
                    srsFlvMuxers[index].setVideoResolution(videoEncoder!!.width, videoEncoder!!.height)
                }
                srsFlvMuxers[index].start(this.urls[index])
            }
        }
    }

    fun startMultiStreamRtpWhenAddPage() {
        if (urls != null) {
            for (index: Int in countUrlOld until urls.size) {
                if (videoEncoder!!.rotation == 90 || videoEncoder!!.rotation == 270) {
                    srsFlvMuxers[index].setVideoResolution(videoEncoder!!.height, videoEncoder!!.width)
                } else {
                    srsFlvMuxers[index].setVideoResolution(videoEncoder!!.width, videoEncoder!!.height)
                }
                srsFlvMuxers[index].start(urls[index])
            }
        }
    }
    ...
    fun setReTries(reTries: Int) {
        ...
        for (i: Int in 0 until srsFlvMuxers.size) {
            srsFlvMuxers[i].setReTries(reTries)
        }
    }

    fun shouldRetry(reason: String,srsFlvMuxerRetry : SrsFlvMuxer?): Boolean {
        return srsFlvMuxerRetry!!.shouldRetry(reason)
    }

    public fun reConnect(delay: Long,srsFlvMuxerReconnect : SrsFlvMuxer) {
        srsFlvMuxerReconnect.reConnect(delay)
    }
    ...
    fun getAacDataRtp(aacBuffer: ByteBuffer, info: MediaCodec.BufferInfo) {
        for (i: Int in 0 until srsFlvMuxers.size) {
            srsFlvMuxers[i].sendAudio(aacBuffer.duplicate(), info)
        }
    }

    fun onSpsPpsVpsRtp(sps: ByteBuffer, pps: ByteBuffer, vps: ByteBuffer?) {
        for (i: Int in 0 until srsFlvMuxers.size) {
            srsFlvMuxers[i].setSpsPPs(sps.duplicate(), pps)
        }
    }

    fun getH264DataRtp(h264Buffer: ByteBuffer, info: MediaCodec.BufferInfo) {
        for (i: Int in 0 until srsFlvMuxers.size) {
            srsFlvMuxers[i].sendVideo(h264Buffer.duplicate(), info)
        }
    }
    ...
    override fun onConnectionSuccessRtmp() {
        Log.i("onConnectionSuccessRtmp",""  +   srsFlvMuxers.size)
