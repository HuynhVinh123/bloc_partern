...
class Camera(
    ...
    private var publishUrls: ArrayList<String>? = null
    ...
    private fun prepareCameraForRecordAndStream(fps: Int, bitrate: Int?) {
        ...
        for (i in 0 until publishUrls!!.size) {
        ...
        // Turn on audio if it is requested.(Control âm thanh)
        if (enableAudio) {
            rtmpCamera!!.prepareAudio( )
        }

        // Bitrate for the stream/recording. (Control kích thước)
...
//    }
    ...
    /// Trả về giao diện hiển thị camera trên flutter
    fun previewCameraWhenSwitching(result: MethodChannel.Result) {
        ...
        Log.i(TAG, "open: width: " + reply["previewWidth"] + " height: " + reply["previewHeight"] + " currentOrientation: " + currentOrientation + " quarterTurns: " + reply["previewQuarterTurns"] + reply["textureId"] + streamingProfile.videoFrameRate)
        result.success(reply)
    }


    fun switchCamera(cameraDesc: String, result: MethodChannel.Result, width: Int, height: Int) {
        // Used to steam image byte data to dart side
        pictureImageReader = ImageReader.newInstance(
                captureSize.width, captureSize.height, ImageFormat.JPEG, 2)
        try {
            cameraManager.openCamera(cameraDesc, object : CameraDevice.StateCallback() {
                override fun onOpened(device: CameraDevice) {
                    ...
                    previewCameraWhenSwitching(result)
                }
                ...
                override fun onDisconnected(cameraDevice: CameraDevice) {
                    ...
                    close()
                    dartMessenger.send(DartMessenger.EventType.ERROR, "The camera was disconnected.", activity)
                }
                ...
                override fun onError(cameraDevice: CameraDevice, errorCode: Int) {
                    ...
                    dartMessenger.send(DartMessenger.EventType.ERROR, errorDescription, activity)
                }
            }, null)
        ...
        cameraManager.openCamera(
                ...
                object : CameraDevice.StateCallback() {
                    override fun onOpened(device: CameraDevice) {
                        ...
                        reply["previewQuarterTurns"] = currentOrientation / 90
                        Log.i(TAG, "open: width: " + reply["previewWidth"] + " height: " + reply["previewHeight"] + " currentOrientation: " + currentOrientation + " quarterTurns: " + reply["previewQuarterTurns"] + reply["textureId"])
                        result.success(reply)
                    }
    ...
    private fun createCaptureSession(
        ...
        val callback: CameraCaptureSession.StateCallback = object : CameraCaptureSession.StateCallback() {
            ...
            override fun onConfigured(session: CameraCaptureSession) {
                try {
                    if (cameraDevice == null) {
                        dartMessenger.send(
                                DartMessenger.EventType.ERROR, "The camera was closed during configuration.", activity)
                        return
                    }
                    ...
                    dartMessenger.send(DartMessenger.EventType.ERROR, e.message, activity)
                } catch (e: IllegalStateException) {
                    Log.v("Camera", "Error IllHandleError000egalStateException", e)
                    dartMessenger.send(DartMessenger.EventType.ERROR, e.message, activity)
            ...
            override fun onConfigureFailed(cameraCaptureSession: CameraCaptureSession) {
                dartMessenger.send(
                        DartMessenger.EventType.ERROR, "Failed to configure camera session.", activity)
            }
        }
    ...
    fun startVideoStreaming(urls: ArrayList<String>?, bitrate: Int?, result: MethodChannel.Result) {
        publishUrls = urls
        try {
            ...
            if (rtmpCamera == null) {
                ...
                prepareCameraForRecordAndStream(24, bitrate)
    ...
    fun addNewPageLive(urls: ArrayList<String>?, result: MethodChannel.Result) {
        if (urls != null) {
            try {
                ...
                if (rtmpCamera != null) {
                    ...
                    if (publishUrls != null) {
                        publishUrls!!.addAll(urls)
                    }

                    prepareCameraForRecordAndStream(24, 1200 * 1024)
    ...
    fun setReconnectLiveVideoStreaming(result: MethodChannel.Result, bitrate: Int?) {
        onRetryLiveStreaming(result = result, bitrate = bitrate)
    }
    ...
    init {
        ...
        orientationEventListener = object : OrientationEventListener(activity.applicationContext) {
            override fun onOrientationChanged(i: Int) {
                ...
                dartMessenger.send(DartMessenger.EventType.ROTATION_UPDATE, (currentOrientation / 90).toString(), activity)
    ...
    fun onRetryLiveStreaming(result: MethodChannel.Result, bitrate: Int?) {
        rtmpCamera = null
        startVideoStreaming(publishUrls, bitrate, result)
    ...
    override fun onConnectionFailedRtmp(reason: String) {
        if (rtmpCamera != null) {
            ...
            for (i in currentRetries..maxRetries) {
                currentRetries = i
                if (rtmpCamera!!.reTry(3000, reason)) {
                    activity!!.runOnUiThread {
                        dartMessenger.send(DartMessenger.EventType.RTMP_RETRY, reason, activity)
                    }
                    // Success!
                    return
                }
            ...
            rtmpCamera = null
            activity!!.runOnUiThread {
                dartMessenger.send(DartMessenger.EventType.RTMP_STOPPED, "Failed retry", activity)
            }
    ...
    override fun onAuthErrorRtmp() {
        activity!!.runOnUiThread {
            dartMessenger.send(DartMessenger.EventType.ERROR, "Auth error", activity)
        }
    }
    ...
    override fun onDisconnectRtmp() {
        ...
        activity!!.runOnUiThread {
            dartMessenger.send(DartMessenger.EventType.RTMP_STOPPED, "Disconnected", activity)
        }
    }
