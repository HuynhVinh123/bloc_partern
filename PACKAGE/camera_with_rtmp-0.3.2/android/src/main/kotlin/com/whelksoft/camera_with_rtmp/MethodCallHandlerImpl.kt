...
internal class MethodCallHandlerImpl(
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "startVideoStreaming" -> {              Log.i("Stuff", call.arguments.toString())
            ...
            "switchCamera" -> {
                    camera!!.closeWhenSwitchCamera()
                    val cameraName = call.argument<String>("cameraName")
                    camera!!.switchCamera(cameraName!!,result,0,0)
            ...
            "configWidthAndHeight" -> {
                    ...
                    val cameraName = call.argument<String>("descr")
                    camera!!.switchCamera(cameraName!!,result,width,height)
            }
            "reConnect" -> {
                var bitrate: Int? = null
                if (call.hasArgument("bitrate")) {
                ...
                camera!!.setReconnectLiveVideoStreaming(result,bitrate)
            }
            "addPageLive" -> {
                camera!!.addNewPageLive(
                        call.argument("urls"),
                        result)
            }
