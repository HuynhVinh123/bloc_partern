...
class CameraController extends ValueNotifier<CameraValue> {
  ...
  /// Thay đổi camera trước và sau
  Future<void> switchCamera(CameraDescription descr) async {
    try {
      _creatingCompleter = Completer<void>();
      final Map<String, dynamic> reply =
          await _channel.invokeMapMethod<String, dynamic>(
  ...
  Future<void> 
  setWidthAndHeightCamera({int width, int height,CameraDescription descr}) async{
    try {
      await _channel.invokeMethod<void>(
          'configWidthAndHeight', <String, dynamic>{
        'width': width,
        'height': height,
        'descr': descr.name,
      });
  ...
  Future<void> reConnectVideoStreaming( {int bitrate = 1200 * 1024}) async{
    try {
      await _channel.invokeMethod<void>(
          'reConnect', <String, dynamic>{
        'bitrate': bitrate
      });
      return ;
    } on PlatformException catch (e) {
  ...
  void _listener(dynamic event) {
    ...
    print("EventTTTTR $map");
    switch (map['eventType']) {
      case 'error':
        value = value.copyWith(errorDescription: event['errorDescription']);
        print(event['errorDescription'] + 'vinh');
        break;
      case 'camera_closing':
        value = value.copyWith(
  ...
  ///
  /// Throws a [CameraException] if the capture fails.
  Future<void> startVideoStreaming(List<String> urls,
      {int bitrate = 1200 * 1024, bool androidUseOpenGL,int width, int height}) async {
    if (!value.isInitialized || _isDisposed) {
      throw CameraException(
        'Uninitialized CameraController',
        'startVideoStreaming was called on uninitialized CameraController',
  ...
  Future<void> startInsertVideoStreaming(List<String> urls) async {
    try {
      await _channel.invokeMethod<void>(
          'addPageLive', <String, dynamic>{
        'urls': urls,
      });
      // value =
      //     value.copyWith(isStreamingVideoRtmp: true, isStreamingPaused: false);
