...
/*PUBLIC PROPERTY*/
  ...
  Future setLiveCampaign(LiveCampaign value) async {
    // Api thay đổi
    onStateAdd(true, message: "${S.current.updating}..");
    try {
      // update info
    ...
    onStateAdd(false, message: "${S.current.updating}...");
  }
...
/*  METHOD*/
  ...
  Future<void> _fetchFacebookComment() async {
    ...
    final fetchResult = await _fbApi.fetchCommentWithPaging(
        ...
        limit: _setting.getFacebookCommentTake,
        isNewestOnTop: true);
...
//    }
  ...
  Future<void> _checkReatimeStatus() async {
    ...
    if (_currentLiveStatus == null || _currentLiveStatus == LiveStatus.VOD) {
      //stop timer
      _timer?.cancel();
      _dialog.showNotify(message: "Livestream ${S.current.finished}");
...
/* COMMAND*/
  ...
  Future<void> initCommand() async {
    ...
    try {
      ...
      await _checkLiveStatus();
      notifyListeners();
      onStateAdd(true, message: S.current.fetchCustomer);
      await Future.wait([
        _fetchPartner(),
        _fetchOrder(),
      ...
      onStateAdd(true, message: S.current.fetchComments);
  ...
  Future createdOrder(CommentItemModel model) async {
    ...
    while (_createdOrderQuene.isNotEmpty) {
      ...
      _createOrderAndPrint(comment).catchError((e, s) {
        ...
        _dialog.showError(error: e, title: S.current.createFailed);
      ...
      if (order != null) {
        try {
          ...
          await _dialog.showError(
              error: e, title: "${S.current.printFailed}!", isRetry: false);
...
//              dialogResult.type == DialogResultType.RETRY) break retry;
  ...
  Future<void> _createOrderAndPrint(CommentItemModel comment) async {
    ...
    final orderInserted = await _tposApi.insertSaleOnlineOrderFromApp(order);
    // Cập nhật hóa đơn
    ...
    if (_setting.saleOnlinePrintOnlyOneIfHaveOrder && haveOrder) {
      exitFunction();
      return;
    }
  ...
  Future<void> saveComment({bool showError = true}) async {
    ...
    if (fbPost.comment.isEmpty) {
      _dialog.showNotify(message: S.current.noCommentFoundInThisPost);
      return;
    }
    ...
    try {
      ...
      if (showError) {
        _dialog.showError(content: S.current.saveFailed + e.toString());
      }
    }
  }
  ...
  List<CommentItemModel> onSearchFilterComments() {
    final intRegex = RegExp('\\d+', multiLine: true);
    ...
    for (final item in _comments) {
      if (countFilter() > 1) {
        if (_isFilterByCommentHasBuy &&
            item.orderCode != null &&
            item.orderCode != '' &&
            _isFilterByStatus && !_isFilterByCommentHasPhone) {
          ...
          if (isExistStatus) {
            ...
            item.orderCode != null &&
            item.orderCode != '' &&
            _isFilterByCommentHasPhone && !_isFilterByStatus) {
          print( item.orderCode + 'vinh');
          final List<String> phones = intRegex
              .allMatches(item.comment.replaceAll('.', ''))
              .map((RegExpMatch m) => m.group(0))
              .toList();
          for (final phone in phones) {
            final String isExist = getNetworkName(phone);
            if (isExist != 'N/A') {
              _commentSearchs.add(item);
              break;
            }
          }
        } else if (_isFilterByStatus && _isFilterByCommentHasPhone && !_isFilterByCommentHasBuy) {
          final bool isExistStatus =
              partnerStatusReportItems.any((PartnerStatusReportItem element) {
            bool isExist = false;
            if (element.isSelect &&
