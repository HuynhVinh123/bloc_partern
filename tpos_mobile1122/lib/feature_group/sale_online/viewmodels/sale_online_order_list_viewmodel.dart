...
// order
  ...
  Future<void> refreshOrdersCommand() async {
    if (isFetchingOrder) {
      _dialog.showWarning(
          content: "Vui lòng đợi cho quá trình tải dữ liệu hoàn thành");
      return;
    }
    lastResults = null;
    tempOrders?.clear();
  ...
  Future<FastSaleOrderAddEditData> prepareFastSaleOrder() async {
    ...
    if (selectedIds == null || selectedIds.isEmpty) {
      _dialog.showWarning(
          ...
          content: 'Vui lòng chọn một hoặc nhiều đơn hàng để tạo hóa đơn');
    ...
    if (selectedOrders
      ...
      _dialog.showWarning(content: 'Các đơn hàng phải cùng facebook');
      onStateAdd(false, message: "");
      return null;
    }
  ...
  Future<bool> deleteOrder(SaleOnlineOrder order) async {
    try {
      ...
      _dialog.showToast(message: "Đã xóa đơn hàng ${order.code}");
      //remove from list
  ...
  Future changeStatus(SaleOnlineOrder item, String status) async {
    ...
    try {
      ...
      notifyListeners();
      _dialog.showToast(message: "Đã đổi trạng thái đơn hàng");
    } catch (e, s) {
      logger.error("", e, s);
      _dialog.showError(content: e.toString());
    }
...
class SaleOnlineOrderListFilterViewModel extends ViewModel {
  ...
  void _initDefaultSort() {
    ...
    sorts.add(OdataSortItem(field: "Name", dir: "asc"));
    sort = sorts.first;
  }
